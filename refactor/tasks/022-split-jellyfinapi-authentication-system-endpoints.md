# Task 022 — Split JellyfinApi authentication/system endpoints
...
Split JellyfinApi authentication/system endpoints. This task must be behavior-preserving.
...
Create `JellyfinApiAuth.cpp`.
Move system-info/server identity/authentication/token-validation related JellyfinApi method definitions and only their directly coupled file-local helpers.
Keep HttpClient/RouteRequest behavior unchanged.

## Behavior That Must Not Change

- Authentication headers remain identical.
- Server ID verification semantics remain identical.
- Unauthorized classification remains identical.
...
```sh
git add src/net/JellyfinApi.cpp src/net/JellyfinApiAuth.cpp Makefile
git commit -m "refactor: split jellyfin auth api"
git status --short
```
