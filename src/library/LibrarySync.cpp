...
std::future<CatalogDbHierarchyWriteResult> rejectedHierarchyWrite(const char* message)
{
...
library::LibrarySync::stage(const CatalogDbMediaPageWrite& page)
{
...
library::LibrarySync::reconcileSeries(const std::vector<MediaItem>& series, bool authoritative,
...
std::future<CatalogDbHierarchyWriteResult> library::LibrarySync::stageSeriesHierarchy(
    ...
    bool complete, const std::shared_ptr<std::atomic_bool>& cancellation)
{
    if (!complete)
        return rejectedHierarchyWrite("incomplete hierarchy is not eligible for CatalogDb commit");
    if (!m_db)
        return rejectedHierarchyWrite("CatalogDb service is unavailable");
    CatalogDbJobMetadata metadata = m_metadata;
    metadata.cancellation = cancellation ? cancellation : m_cancel;
    return m_db->stageSeriesHierarchy(series, seasons, episodesBySeason, generation,
                                      static_cast<std::int64_t>(std::time(nullptr)) * 1000, true,
                                      metadata);
}

std::future<CatalogDbSyncState>
library::LibrarySync::writeSyncState(std::int64_t lastSuccessfulMs, std::int64_t lastReconcileMs,
                                     std::uint64_t committedGeneration,
                                     const std::shared_ptr<std::atomic_bool>& cancellation)
{
    CatalogDbJobMetadata metadata = m_metadata;
    metadata.cancellation = cancellation ? cancellation : m_cancel;
    return m_db->writeSyncState(lastSuccessfulMs, lastReconcileMs, committedGeneration, metadata);
}

std::future<library::OfflineRebuildResult>
library::LibrarySync::reconstructOfflineDownloads(const std::string& downloadRoot)
{
    const auto db = m_db;
    ...
    const auto operationCancellation = std::make_shared<std::atomic_bool>(false);
    {
    ...
    return std::async(std::launch::async, [db, session, metadata, cancellation,
                                           reconstructionGeneration, operationCancellation,
                                           generation, downloadRoot] {
        ...
        const std::string scope = DownloadStore::scopeKey(session.serverUrl, session.userId);
        ...
        if (!store.loadCompleteMetadata(scope, downloads, &error)) {
            ...
            result.message = error.empty() ? "download metadata could not be read" : error;
            return result;
        }

        const OfflineLibraryQuery::Hierarchy hierarchy = OfflineLibraryQuery::hierarchy(downloads);
        CatalogDbJobMetadata writeMetadata = metadata;
        writeMetadata.cancellation = operationCancellation;
        const auto write = [&](const MediaItem& root, const std::vector<MediaItem>& seasons,
                               const std::map<std::string, std::vector<MediaItem>>& episodes) {
            if (!current())
                return false;
            const auto written =
                db->stageSeriesHierarchy(root, seasons, episodes, 0, 0, false, writeMetadata).get();
        ...
        result.containersSynthesized = hierarchy.series.size() + hierarchy.seasons.size();
        for (const auto& seriesEntry : hierarchy.series) {
            ...
            for (const auto& seasonEntry : hierarchy.seasons) {
                ...
                const auto found = hierarchy.episodesBySeason.find(seasonEntry.first);
                episodes.emplace(seasonEntry.first, found == hierarchy.episodesBySeason.end()
