...
{
    switch (reason) {
    case DownloadInterrupt::Playback:
        return DownloadState::PausedForPlayback;
    case DownloadInterrupt::UserPause:
...
{
    if (!chunkSize || index >= chunkCount(size, chunkSize))
        return {};
...
inline std::uint64_t chunkLength(std::uint64_t size, std::uint64_t index,
                                 std::uint64_t chunkSize = DOWNLOAD_CHUNK_SIZE)
{
    ByteRange r = chunkRange(size, index, chunkSize);
    return r.valid ? r.end - r.start + 1 : 0;
}
...
{
    return a && b > (std::numeric_limits<std::uint64_t>::max)() / a
               ? (std::numeric_limits<std::uint64_t>::max)()
               : a * b;
}
inline std::string formatBytes(std::uint64_t bytes)
{
    ...
    std::snprintf(out, sizeof(out), n >= 10.0 || unit[0] == 'B' ? "%.0f %s" : "%.1f %s", n, unit);
    return out;
}
...
struct DownloadItem
{
    ...
    std::uint64_t expectedSize = 0, chunkSize = DOWNLOAD_CHUNK_SIZE, downloadedBytes = 0,
                  bitrate = 0;
    ...
    std::uint64_t hlsCompletedSegments = 0, hlsCurrentSegmentBytes = 0, hlsCurrentSegmentSize = 0;
...
{
    std::vector<DownloadItem> items;
    std::uint64_t totalSourceBytes = 0, alreadyPresentBytes = 0, additionalRequiredBytes = 0,
                  filesystemFreeBytes = 0, alreadyReservedBytes = 0, usableFreeBytes = 0;
    bool sizeKnown = false, canFit = false;
    std::string error;
};
struct DownloadPlanSnapshot
{
    std::uint64_t id = 0;
    DownloadPlanState state = DownloadPlanState::Idle;
    std::size_t itemCount = 0;
    DownloadPlan plan;
};
inline std::uint64_t remainingBytes(const DownloadItem& i)
{
    return i.downloadedBytes >= i.expectedSize ? 0 : i.expectedSize - i.downloadedBytes;
}
...
inline std::uint64_t queueRemainingBytes(const DownloadItem& i)
{
    ...
    if (i.hlsCurrentSegmentSize) {
        const std::uint64_t currentRemaining =
            i.hlsCurrentSegmentSize > current ? i.hlsCurrentSegmentSize - current : 0;
        if (remainingSegments == 1)
            return currentRemaining;
        const std::uint64_t average =
            completed ? completedBytes / completed : i.hlsCurrentSegmentSize;
...
{
    if (i.hlsStorage)
        return estimateHlsBytes(i.runtimeTicks, bytes);
    bytes = i.expectedSize;
    return bytes != 0;
}
...
inline const char* downloadStateLabel(DownloadState s)
{
    switch (s) {
    case DownloadState::Queued:
        return "Queued";
    case DownloadState::Planning:
        return "Planning";
    case DownloadState::Downloading:
        return "Downloading";
    case DownloadState::Paused:
        return "Paused";
    case DownloadState::PausedForPlayback:
    ...
    case DownloadState::Unauthorized:
        return "Unauthorized";
    case DownloadState::ServerMissing:
        return "Server missing";
    case DownloadState::UpdateAvailable:
        return "UPDATE AVAILABLE";
    case DownloadState::LocalOnly:
        return "LOCAL ONLY";
    case DownloadState::NoSpace:
        return "No space";
    default:
        return "Failed";
    }
