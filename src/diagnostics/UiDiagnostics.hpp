...
class UiDiagnostics
{
  public:
    ...
    struct Watchdog
    {
        uint64_t seen = 0;
        bool stalled = false;
        // Returns 1 for begin, 2 for end, 0 otherwise.  `suspended` suppresses and resets a pending
        // stall.
    ...
    {
        m_phase.store(phase, std::memory_order_relaxed);
    }
    ...
    {
        m_screen.store(screen, std::memory_order_relaxed);
    }
    const char* screenName() const noexcept
    {
        return m_screen.load(std::memory_order_relaxed);
    }
    void setTab(const char* tab)
    {
        m_tab.store(tab, std::memory_order_relaxed);
    }
    const char* tabName() const noexcept
    {
        return m_tab.load(std::memory_order_relaxed);
    }
    void setLastAction(const char* action)
    {
        m_action.store(action, std::memory_order_relaxed);
    }
    const char* actionName() const noexcept
    {
        return m_action.load(std::memory_order_relaxed);
    }
    void setScope(const char* scope)
    {
        m_scope.store(scope, std::memory_order_relaxed);
    }
    const char* scopeName() const noexcept
    {
        return m_scope.load(std::memory_order_relaxed);
    }
    const char* exchangeScope(const char* scope)
    {
        return m_scope.exchange(scope, std::memory_order_relaxed);
    }
    void setSuspended(bool suspended)
    {
        m_suspended.store(suspended, std::memory_order_relaxed);
    }
  ...
  private:
    ...
    void slow(const char* name, uint64_t elapsed);
    std::atomic<uint64_t> m_heartbeat{0};
    std::atomic<bool> m_stop{false}, m_suspended{false};
    std::atomic<uint16_t> m_activeWorkerMask{0};
    std::atomic<const char*> m_phase{"idle/frame boundary"}, m_screen{"none"}, m_tab{"n/a"},
        m_action{"None"}, m_scope{""};
    std::atomic<const char*> m_library{"idle"}, m_hierarchy{"idle"}, m_artwork{"idle"},
        m_download{"idle"};
    std::thread m_thread;
    std::string m_path;
