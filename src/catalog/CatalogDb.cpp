...
namespace miyoofin {
// Static startup timeline markers retained across the CatalogDb query split:
...
std::uint64_t CatalogDb::configureScope(const std::string& serverUrl, const std::string& userId)
{
    const bool validIdentity = validScopeIdentity(serverUrl, userId);
    const std::string scopeKey =
        validIdentity ? catalog::scopeKey(serverUrl, userId) : std::string();
    std::uint64_t epoch;
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        ...
        m_scopeReady = false;
        m_scopeStatus =
            validIdentity ? CatalogDbScopeStatus::Pending : CatalogDbScopeStatus::InvalidIdentity;
        m_lastError =
            validIdentity ? CatalogDbErrorCategory::None : CatalogDbErrorCategory::InvalidIdentity;
        m_openState = CatalogDbOpenState::NotAttempted;
        m_migrationState = {};
        m_scopeCommands.clear();
        m_scopeCommands.push_back(
            {validIdentity ? ScopeCommandKind::Configure : ScopeCommandKind::InvalidIdentity, epoch,
             scopeKey});
    }
    char line[256];
    std::snprintf(
        line, sizeof(line), "configure_scope_requested epoch=%llu scope_hash=%s identity=%s",
        static_cast<unsigned long long>(epoch), scopeKey.empty() ? "none" : scopeKey.c_str(),
...
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_populationStatus;
}

std::future<CatalogCompatibilitySeedResult>
CatalogDb::enqueueLibrarySeed(const CatalogCompatibilitySeedRequest& request,
                              const CatalogDbJobMetadata& metadata, CatalogDbFailureSpec injection)
{
    ...
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_stopping) {
        CatalogCompatibilitySeedResult r;
    ...
    if (m_pendingJobs >= kMaxPendingJobs) {
        ...
        r.message = "CatalogDb library seed queue is full";
        command->result.set_value(std::move(r));
        return future;
    }
    ...
    m_librarySeedCommands.push_back(command);
    ++m_pendingJobs;
...
void CatalogDb::workerLoop()
{
    for (;;) {
        ...
        m_wake.wait(lock, [this] {
            ...
            return m_stopping ||
                   (!pausedForTest &&
                    (!m_scopeCommands.empty() || hasTestCommands || !m_syncStateCommands.empty() ||
                     !m_librarySeedCommands.empty() || !m_libraryReadCommands.empty() ||
                     !m_mediaPageCommands.empty() || !m_mediaPageUpsertCommands.empty() ||
                     !m_topLevelSyncCommands.empty() || hasPendingJobsLocked()));
        });
        ...
        if (!m_writeCommands.empty()) {
            std::shared_ptr<HierarchyWriteCommand> command = std::move(m_writeCommands.front());
        ...
        if (!m_reconcileCommands.empty()) {
            std::shared_ptr<ReconcileCommand> command = std::move(m_reconcileCommands.front());
        ...
        if (!m_syncStateCommands.empty()) {
            std::shared_ptr<SyncStateCommand> command = std::move(m_syncStateCommands.front());
        ...
        if (!m_librarySeedCommands.empty()) {
            ...
            m_librarySeedCommands.pop_front();
            --m_pendingJobs;
            performanceTelemetry().setCatalogDbQueueDepth(static_cast<uint32_t>(m_pendingJobs));
            m_runningJob = true;
            performanceTelemetry().setCatalogDbActive(true);
        ...
        if (!m_libraryReadCommands.empty()) {
            auto command = std::move(m_libraryReadCommands.front());
            m_libraryReadCommands.pop_front();
            --m_pendingJobs;
            m_runningJob = true;
            performanceTelemetry().setCatalogDbActive(true);
            lock.unlock();
            processLibraryRead(command);
            lock.lock();
            performanceTelemetry().setCatalogDbActive(false);
            m_runningJob = false;
            m_idle.notify_all();
        ...
        if (!m_mediaPageCommands.empty()) {
            auto command = std::move(m_mediaPageCommands.front());
            m_mediaPageCommands.pop_front();
            --m_pendingJobs;
            m_runningJob = true;
            performanceTelemetry().setCatalogDbActive(true);
            lock.unlock();
            processMediaPage(command);
            lock.lock();
            performanceTelemetry().setCatalogDbActive(false);
            m_runningJob = false;
            m_idle.notify_all();
        ...
        if (!m_mediaPageUpsertCommands.empty()) {
            auto command = std::move(m_mediaPageUpsertCommands.front());
            m_mediaPageUpsertCommands.pop_front();
            --m_pendingJobs;
            m_runningJob = true;
            performanceTelemetry().setCatalogDbActive(true);
            lock.unlock();
            processMediaPageUpsert(command);
            lock.lock();
            performanceTelemetry().setCatalogDbActive(false);
            m_runningJob = false;
            m_idle.notify_all();
        ...
        if (!m_topLevelSyncCommands.empty()) {
            ...
            m_topLevelSyncCommands.pop_front();
            --m_pendingJobs;
            m_runningJob = true;
            performanceTelemetry().setCatalogDbActive(true);
            lock.unlock();
            processTopLevelSync(command);
            lock.lock();
            performanceTelemetry().setCatalogDbActive(false);
            m_runningJob = false;
            m_idle.notify_all();
        ...
        if (job.metadata.cancellation && job.metadata.cancellation->load()) {
            ...
            if (job.metadata.generation != m_generation ||
                (job.metadata.scopeEpoch != 0 && (job.metadata.scopeEpoch != m_requestedEpoch ||
        ...
        if (job.enqueuedMonotonicUs != 0 && completedUs >= job.enqueuedMonotonicUs)
            performanceTelemetry().recordCatalogDbQueueWait(completedUs - job.enqueuedMonotonicUs);
...
void CatalogDb::processScopeCommand(ScopeCommand command)
{
    {
        char line[256];
        std::snprintf(line, sizeof(line), "scope_command_started epoch=%llu scope_hash=%s kind=%u",
    ...
    const CatalogDbMigrationState migrationState = inspectMigrationState(command.scopeKey);
    ...
    }
    {
        ...
        std::snprintf(line, sizeof(line),
                      "state epoch=%llu scope_hash=%s db_dir=%s final_present=%d "
                      "migrating_present=%d bootstrap_needed=%d decision=%s(%u) path_error=%d",
                      static_cast<unsigned long long>(command.epoch), command.scopeKey.c_str(),
                      scopeDirectory(command.scopeKey).c_str(), migrationState.finalPresent ? 1 : 0,
                      migrationState.migratingPresent ? 1 : 0, bootstrapNeeded ? 1 : 0,
    ...
    const CatalogDbMigrationState completedState = inspectMigrationState(command.scopeKey);
    ...
    if (bootstrapNeeded) {
        ...
        catalogDiagnostic(std::string("bootstrap_job_completed success=") + (opened ? "1" : "0"));
    }
