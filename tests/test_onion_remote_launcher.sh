...
grep -q 'miyoofin_force_full_boot_once' "$ROOT/tools/miyoo/miyoofin-reboot.c" || fail 'reboot helper does not create full-boot marker'
grep -q 'miyoofin_force_full_boot_once' "$ROOT/tools/miyoo/install-charging-boot-marker.sh" || fail 'charging marker installer is missing'
grep -q 'unexpected_runtime_hook' "$ROOT/tools/miyoo/install-charging-boot-marker.sh" || fail 'charging marker installer lacks hook replacement guard'
grep -q 'install-charging-boot-marker.sh' "$ROOT/tools/miyoo/miyoofin-dropbear-2222.sh" || fail 'startup does not verify charging marker hook'
...
# Architecture boundaries (class A): online Home startup must not fall back to
# the full-snapshot read / blocking seed, and the catalog scope must be
# configured before Home is entered.  The bounded paging and cancellation
# wiring is covered behaviourally by test_home_library_controller and the
# coordinator/query tests.
