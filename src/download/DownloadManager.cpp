...
std::uint64_t DownloadManager::requestPlan(const std::vector<MediaItem>&items){std::vector<DownloadItem> provisional;for(const auto&m:items){DownloadItem i;i.itemId=m.id;i.itemType=m.type;i.title=m.title;i.runtimeTicks=m.runTimeTicks;i.hlsStorage=true;provisional.push_back(i);}DownloadPlan estimate=makePlan(provisional);std::lock_guard<std::mutex>l(m_mutex);std::uint64_t id=m_nextPlanId++;DownloadPlanSnapshot s;s.id=id;s.state=DownloadPlanState::Planning;s.itemCount=items.size();s.plan=estimate;m_plans[id]=s;m_planJobs.push_back({id,m_generation,m_session,items,"","",{}, {}});m_planWake.notify_one();return id;}
...
DownloadPlanSnapshot DownloadManager::planSnapshot(std::uint64_t id)const{std::lock_guard<std::mutex>l(m_mutex);auto it=m_plans.find(id);return it==m_plans.end()?DownloadPlanSnapshot{}:it->second;}
void DownloadManager::reconciler(){for(;;){Session session;std::string scope;std::uint64_t generation=0;std::vector<DownloadItem> items;{std::unique_lock<std::mutex>l(m_mutex);m_reconcileWake.wait(l,[&]{return m_stop||m_reconcileRequested;});if(m_stop)return;m_reconcileWake.wait_for(l,std::chrono::seconds(3),[&]{return m_stop;});if(m_stop)return;m_reconcileRequested=false;session=m_session;scope=m_scope;generation=m_generation;items=m_items;}for(const auto&old:items){if(!session.valid())break;std::vector<DownloadMediaSource> sources;std::string error;SourceCheck result=SourceCheck::Transient;if(JellyfinApi::getDownloadMediaSources(session.serverUrl,session.accessToken,session.userId,session.deviceId,old.itemId,sources,error)&&!sources.empty())result=SourceCheck::Same;else if(error.find("HTTP 404")!=std::string::npos)result=SourceCheck::Missing;else if(error=="Unauthorized")result=SourceCheck::Unauthorized;const DownloadMediaSource*source=result==SourceCheck::Same?&sources.front():nullptr;std::lock_guard<std::mutex>l(m_mutex);if(m_stop||generation!=m_generation||scope!=m_scope)break;auto p=std::find_if(m_items.begin(),m_items.end(),[&](const DownloadItem&i){return i.itemId==old.itemId;});if(p==m_items.end()||p->state==DownloadState::Downloading)continue;bool discard=reconcileSource(*p,result,source);if(discard)m_store.removePartialBytes(scope,p->itemId,nullptr);m_store.saveManifest(scope,*p,nullptr);persistLocked();}}}
void DownloadManager::planner(){for(;;){PlanJob job;{std::unique_lock<std::mutex>l(m_mutex);m_planWake.wait(l,[&]{return m_stop||!m_planJobs.empty();});if(m_stop)return;job=std::move(m_planJobs.front());m_planJobs.pop_front();}std::vector<MediaItem> media=job.items,seasons;std::map<std::string,std::vector<MediaItem> > hierarchy;std::string error;if(!job.seasonId.empty()){if(JellyfinApi::getEpisodes(job.session.serverUrl,job.session.accessToken,job.session.userId,job.session.deviceId,job.seriesId,job.seasonId,media,error)){MediaItem season=job.season;season.id=job.seasonId;season.seriesId=job.seriesId;seasons.push_back(season);hierarchy[job.seasonId]=media;}}else if(!job.seriesId.empty()){if(JellyfinApi::getSeasons(job.session.serverUrl,job.session.accessToken,job.session.userId,job.session.deviceId,job.seriesId,seasons,error))for(const auto&season:seasons){std::vector<MediaItem> episodes;if(!JellyfinApi::getEpisodes(job.session.serverUrl,job.session.accessToken,job.session.userId,job.session.deviceId,job.seriesId,season.id,episodes,error))break;hierarchy[season.id]=episodes;media.insert(media.end(),episodes.begin(),episodes.end());}}if(error.empty()&&!job.seriesId.empty()){MediaItem series=job.series;series.id=job.seriesId;if(series.title.empty())for(const auto&a:hierarchy)if(!a.second.empty()){series.title=a.second.front().seriesName;break;}OfflineCatalog::storeDiscoveredHierarchy(OfflineCatalog::cachePath("cache",LibraryCache::scopeKey(job.session.serverUrl,job.session.userId)),series,seasons,hierarchy,true,nullptr);}std::vector<DownloadItem> out;std::set<std::string> seen;for(const auto&m:media){if(!seen.insert(m.id).second)continue;std::vector<DownloadMediaSource> sources;if(!JellyfinApi::getDownloadMediaSources(job.session.serverUrl,job.session.accessToken,job.session.userId,job.session.deviceId,m.id,sources,error)||sources.empty()){if(error.empty())error="No downloadable media source";break;}out.push_back(makeDownloadItem(m,sources.front()));}DownloadPlan calculated; if(error.empty()) calculated=makePlan(out);std::lock_guard<std::mutex>l(m_mutex);auto it=m_plans.find(job.id);if(it==m_plans.end()||job.generation!=m_generation)continue;it->second.itemCount=out.size();if(!error.empty()){it->second.state=DownloadPlanState::Error;it->second.plan.error=error;}else{it->second.plan=calculated;it->second.state=calculated.error.empty()?DownloadPlanState::Ready:DownloadPlanState::Error;}}}
...
bool DownloadManager::transfer(DownloadItem&item,const Session&session,const std::string&scope,std::uint64_t generation){
    ...
    if(!JellyfinApi::getHlsSegmentUrls(session.serverUrl,session.accessToken,session.deviceId,item.itemId,item.mediaSourceId,urls,error,&failure)){
