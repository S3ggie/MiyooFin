    ...
    if (m_downloads && !m_prepareCancelled.load(std::memory_order_acquire)) {
        std::uint64_t planId = 0;
        {
            ...
            UiDiagnostics::Scope scope(
                "MovieDetailsScreen::DownloadManager snapshot/free-space preparation", false);
...
DecodedImage MovieDetailsScreen::loadMovieArtwork()
{
    ...
    if (cached) {
        uiDiagnostics().setWorker("artwork", "movie ImageCache read");
        {
            ...
            jpegData =
                ImageCache::readCached(m_movie.id, ImageType::Primary, tag, POSTER_W, POSTER_H);
        }
    ...
    if (jpegData.empty()) {
        ...
        auto headers = JellyfinApi::buildAuthHeaders(m_session.accessToken, m_session.deviceId);
        ...
        bool fetched = false;
        {
            ...
            fetched = RouteRequest(m_session).run(
                [&](const std::string& base) {
                    return client.getBinary(buildImageUrl(base, m_movie.id, ImageType::Primary, tag,
                                                          POSTER_W, POSTER_H),
                                            headers, response, error, 512 * 1024,
                                            &m_prepareCancelled) &&
                           response.ok();
                },
                error);
        ...
        if (!m_prepareCancelled.load(std::memory_order_acquire)) {
            ...
            ImageCache::writeToCache(m_movie.id, ImageType::Primary, tag, POSTER_W, POSTER_H,
