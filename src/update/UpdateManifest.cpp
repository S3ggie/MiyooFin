...
static std::string scopedRaw(const std::string &scope,
                             const std::string &key)
{
    ...
    if (start == '"') {
        ...
        if (pos < scope.size()) pos++; // past closing quote
        ...
        while (pos < scope.size() && depth > 0) {
            ...
            } else if (scope[pos] == start) depth++;
            else if (scope[pos] == close) depth--;
            pos++;
        }
        return scope.substr(s, pos - s);
    } else {
        ...
        size_t s = pos;
        while (pos < scope.size() && scope[pos] != ',' && scope[pos] != '}'
               && scope[pos] != ']' && scope[pos] != ' '
               && scope[pos] != '\n' && scope[pos] != '\t'
               && scope[pos] != '\r')
            pos++;
...
static std::string nestedObject(const std::string &json,
    ...
    if (ppos < json.size() && json[ppos] == '{') {
        ...
        for (size_t i = ppos; i < json.size(); i++) {
            if (inStr) {
                ...
                if (json[i] == '"') inStr = false;
            } else {
                if (json[i] == '"') inStr = true;
...
static bool normalizeSha256(const std::string &in, std::string &out)
{
    ...
    for (char c : in) {
        ...
        else if (c >= 'A' && c <= 'F') out += static_cast<char>(c + 32);
        else return false;
    }
    return true;
}
...
bool parseUpdateManifest(const std::string &json, UpdateManifest &out)
{
    ...
    std::string shaNorm;
    if (!normalizeSha256(tarSha, shaNorm)) return false;
