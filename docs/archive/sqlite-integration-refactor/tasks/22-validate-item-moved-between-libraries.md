# Task 22 — Validate item moved between libraries

**Phase:** C — validation

## Objective

Prove one item can move from one supported view to another across generations without duplicate/stale membership.

## Why

Membership, not historical media kind, must be authoritative across moves.
...
- `tests/cases/test_ui_foundation.inc` only if classification is asserted
...
1. Commit generation A with item in view A.
2. Commit generation B with same item only in view B.
3. Read item page and memberships.

## Tests

- Item appears once.
- Membership contains B not A.
- Cursor/order stays stable.
...
```text
test(catalog): cover item library move
```

Do not include any part of Task 23 in this commit.
