...
void SeriesScreen::fetchSeasons(bool loadCachedSeasons)
{
    ...
    const bool networkOffline = m_networkOffline, downloadedOnly = m_downloadedOnly,
               loadCached = loadCachedSeasons;
    ...
    m_fetchThread = std::thread([this, id, networkOffline, downloadedOnly, loadCached, downloads,
        ...
        if (loadCached) {
            ...
            if (libraryQuery) {
                const library::HierarchyPage result = libraryQuery->seasons(id, cancellation).get();
                if (result.success)
                    cached = result.items;
            ...
            if (downloadedOnly) {
                ...
                for (const auto& season : cached) {
                    for (const auto& download : snapshot.items) {
                        const bool complete = download.state == DownloadState::Complete ||
                                              download.state == DownloadState::LocalOnly ||
                                              download.state == DownloadState::UpdateAvailable;
                        if (complete && download.itemType == "episode" && download.seriesId == id &&
                ...
                if (filtered.empty()) {
                    for (const auto& download : snapshot.items) {
                        ...
                        if (!complete || download.itemType != "episode" ||
                            download.seriesId != id || download.seasonId.empty())
                            continue;
                        MediaItem season;
                        ...
                        if (!duplicate)
                            filtered.push_back(std::move(season));
                    }
                }
            ...
            {
                std::lock_guard<std::mutex> g(m_fetchMutex);
                m_cachedSeasons = std::move(cached);
                m_cachedSeasonsDone = true;
            }
            if (networkOffline) {
                std::lock_guard<std::mutex> g(m_fetchMutex);
                m_fetchOk = true;
...
void SeriesScreen::queueArtwork(const std::string& key, const MediaItem& item, int width,
                                int height, bool series)
{
    {
        std::lock_guard<std::mutex> g(m_artworkMutex);
        m_artworkJobs.push_back({key, item, width, height, series});
        if (!m_artworkThread.joinable())
            m_artworkThread = std::thread(&SeriesScreen::artworkWorkerLoop, this);
    }
...
void SeriesScreen::artworkWorkerLoop()
{
    ...
    for (;;) {
        ArtworkJob job;
        {
            std::unique_lock<std::mutex> l(m_artworkMutex);
            m_artworkCv.wait(l, [&] { return m_artworkStop || !m_artworkJobs.empty(); });
            if (m_artworkStop)
                return;
            job = std::move(m_artworkJobs.front());
            m_artworkJobs.erase(m_artworkJobs.begin());
        }
        auto tag = job.item.imageTags.find("Primary");
        if (tag == job.item.imageTags.end())
            continue;
        std::vector<unsigned char> data;
        if (ImageCache::isCached(job.item.id, ImageType::Primary, tag->second, job.width,
                                 job.height))
            data = ImageCache::readCached(job.item.id, ImageType::Primary, tag->second, job.width,
                                          job.height);
        if (data.empty() && !m_artworkCancelled.load(std::memory_order_acquire)) {
            ...
            if (RouteRequest(m_session).run(
                    [&](const std::string& base) {
                        return artworkClient.getBinary(
                                   buildImageUrl(base, job.item.id, ImageType::Primary, tag->second,
                                                 job.width, job.height),
                                   JellyfinApi::buildAuthHeaders(m_session.accessToken,
                                                                 m_session.deviceId),
                                   r, e, 512 * 1024, &m_artworkCancelled) &&
                               r.ok();
                    },
                    e)) {
                data = std::move(r.data);
                ImageCache::writeToCache(job.item.id, ImageType::Primary, tag->second, job.width,
                                         job.height, data.data(), data.size());
            }
        }
        DecodedImage image;
        if (!data.empty() && !m_artworkCancelled.load(std::memory_order_acquire))
            image = ImageDecoder::decodeJpeg(data.data(), data.size());
        {
            std::lock_guard<std::mutex> g(m_artworkMutex);
            if (!m_artworkStop)
                m_artworkCompleted[job.key] = std::move(image);
        }
    }
}
