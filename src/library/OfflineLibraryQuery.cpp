...
void addContainer(std::map<std::string, MediaItem>& items, const std::string& id,
                  const std::string& type, const std::string& title, const std::string& seriesId,
                  std::int32_t index)
{
...
OfflineLibraryQuery::Hierarchy
OfflineLibraryQuery::hierarchy(const std::vector<DownloadItem>& downloads)
{
    Hierarchy result;
    ...
    for (const auto& download : ordered) {
        ...
        if (download.itemType == "movie") {
            ...
            movie.type = "movie";
            movie.title = download.title.empty() ? download.itemId : download.title;
        ...
        if (download.itemType != "episode" || download.seriesId.empty() ||
            download.seasonId.empty()) {
            continue;
        }
        addContainer(result.series, download.seriesId, "show", download.seriesName, {}, 0);
        addContainer(result.seasons, download.seasonId, "season", download.seasonName,
                     download.seriesId, download.seasonNumber);
        ...
        episode.progress = episode.runTimeTicks > 0
                               ...
                               : 0.0f;
        result.episodesBySeason[download.seasonId].push_back(std::move(episode));
    }
...
OfflineLibraryQuery::metadataBatches(const DownloadSnapshot& downloads)
{
    std::vector<std::string> ids;
    std::set<std::string> seen;
    for (const auto& download : downloads.items) {
        if (!isAvailable(download.state))
            continue;
                                           ...
                                           const std::vector<MediaItem>& metadata)
{
    ...
    for (const auto& download : downloads.items) {
        if (!isAvailable(download.state) || download.itemId.empty())
            continue;
        MediaItem item =
            canonical.count(download.itemId) ? canonical[download.itemId] : fallback(download);
        if (item.type == "episode") {
            if (!item.seriesId.empty()) {
                ...
                if (found != canonical.end()) {
                    ...
                    series.title =
                        download.seriesName.empty() ? "Downloaded Show" : download.seriesName;
                }
