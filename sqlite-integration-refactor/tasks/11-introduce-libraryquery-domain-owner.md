# Task 11 — Introduce the LibraryQuery domain owner

**Phase:** B — ownership/modularity

## Objective

Introduce one lightweight query component for domain-level SQLite catalog reads.

## Why

Screens currently understand raw CatalogDb futures, memberships, scope metadata, and legacy cache-shaped results.

## Preconditions

- Task 10 LibrarySync exists.
- CatalogDb remains single SQLite executor.

## Allowed Files

- Create `src/library/LibraryQuery.hpp`
- Create `src/library/LibraryQuery.cpp`
- `src/catalog/CatalogDb.hpp`/`.cpp` only if a lower bounded primitive is missing
- `Makefile`
- `Makefile.cross`
...
- Do not add another DB worker/connection.
- Do not add `ILibraryRepository`, DI container, generic framework.
- Do not migrate consumers yet except minimal compile seams.
- Do not materialize complete libraries.
...
1. LibraryQuery holds shared CatalogDb + active scope epoch.
2. Expose domain queries for committed view descriptors, bounded movie/show pages, seasons, episodes, and bounded metadata-by-ID.
3. Public LibraryQuery results must not mention CachedLibraryView, LibrarySnapshot, SDL, or SQLite statement details.
4. LibraryQuery may return futures fulfilled by CatalogDb; it creates no DB thread.
5. Consumers cannot configure CatalogDb scope through LibraryQuery.
6. Preserve existing bounded page caps/cursor behavior.

## Tests

- Domain result mapping preserves success/cancel/superseded/error.
- Movie/show cursor and membership behavior matches Task 05.
- Hierarchy order remains deterministic.
- LibraryQuery never opens SQLite directly.
...
```text
refactor(library): add catalog query owner
```

Do not include any part of Task 12 in this commit.
