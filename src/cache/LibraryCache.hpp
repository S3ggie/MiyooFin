...
namespace miyoofin {
struct CachedLibraryView
{
    std::string id, name, collectionType;
...
{
    int added = 0, changed = 0, removed = 0, unchanged = 0, postersNeeded = 0, stalePosters = 0;
};
struct StalePoster
{
...
class LibraryCache
{
  public:
    ...
    static bool load(const std::string& path, LibrarySnapshot& snapshot,
                     std::string* error = nullptr, bool* needsRefresh = nullptr);
    static ReconcileStats reconcile(const LibrarySnapshot& oldSnapshot,
                                    const LibrarySnapshot& remoteSnapshot,
