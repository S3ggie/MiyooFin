# Task 28 — Validate cancellation halfway through population

**Phase:** C — validation

## Objective

Prove cancelling a new generation after staged pages preserves the entire previous committed generation.

## Why

Rollback/preservation is the core reason for generation staging.
...
- `tests/cases/test_catalog_migration.inc` if cancellation fixtures are reused
...
1. Commit generation A with multiple views/items.
2. Begin generation B and stage at least one differing page.
3. Cancel before finalize and allow abort cleanup.
4. Query committed views/pages, then start generation C.

## Tests

- A remains exactly visible.
- No B staged-only row leaks.
- C can begin/finalize cleanly.
...
```text
test(catalog): cover cancelled population rollback
```

Do not include any part of Task 29 in this commit.
