...
static void bounded_sync(void)
{
    ...
    if (child < 0) {
        recovery_log("sync_fork_failed");
        return;
    }
    for (int i = 0; i < SYNC_TIMEOUT_SECONDS * 10; ++i) {
        int status = 0;
        if (waitpid(child, &status, WNOHANG) == child) {
            recovery_log(WIFEXITED(status) && WEXITSTATUS(status) == 0 ? "sync_complete"
                                                                       : "sync_failed");
            return;
        }
        usleep(100000);
...
static int require_one_mainui(void)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        for (const char* p = name; *p; ++p)
            if (*p < '0' || *p > '9')
                numeric = 0;
        if (numeric && process_mainui(name))
            ++count;
    }
    closedir(dir);
    if (count != 1)
        return fail("expected exactly one validated MainUI process");
    return 0;
}
...
int main(int argc, char** argv)
{
    ...
    if (argc != 1 && !recovery)
        return fail("only --recovery is accepted");
    if (getuid() != EXPECTED_ONION_UID)
        return fail("caller is not onion");
    if (geteuid() != 0)
        return fail("helper is not setuid-root");
    unsetenv("PATH");
    if (recovery) {
        ...
        recovery_log(path_exists(QUEUE_PATH) || path_exists(ONION_QUEUE) ? "launch_queue_present"
                                                                         : "launch_queue_absent");
        recovery_log(handoff_in_progress() ? "handoff_in_progress" : "handoff_idle");
        if (!create_full_boot_marker())
            return fail("could not create full-boot marker");
        bounded_sync();
    ...
    if (any_comm("miyoofin"))
        return fail("MiyooFin is still running");
    if (require_one_mainui() != 0)
        return 1;
    if (path_exists(QUEUE_PATH) || path_exists(ONION_QUEUE))
        return fail("an Onion launch queue is staged");
    if (handoff_in_progress())
        return fail("a developer handoff is in progress");
    if (!create_full_boot_marker())
        return fail("could not create full-boot marker");
    sync();
    if (reboot(RB_AUTOBOOT) != 0) {
