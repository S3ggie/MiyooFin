...
std::string ImageCache::cacheFilename(const std::string& itemId, ImageType type,
                                      const std::string& imageTag, int width, int height)
{
    char buf[512];
    std::snprintf(buf, sizeof(buf), "%s_%s_%s_%dx%d.jpg", itemId.c_str(), imageTypeToString(type),
                  imageTag.c_str(), width, height);
    return std::string(buf);
}

std::string ImageCache::cachePath(const std::string& itemId, ImageType type,
                                  const std::string& imageTag, int width, int height)
{
    return s_cacheDir + cacheFilename(itemId, type, imageTag, width, height);
}

bool ImageCache::isCached(const std::string& itemId, ImageType type, const std::string& imageTag,
...
std::vector<unsigned char> ImageCache::readCached(const std::string& itemId, ImageType type,
...
bool ImageCache::writeToCache(const std::string& itemId, ImageType type,
                              const std::string& imageTag, int width, int height,
...
std::vector<std::size_t> ImageCache::selectCacheVictims(std::vector<JanitorFileEntry>& entries,
                                                        std::int64_t targetBytes)
{
    ...
    std::stable_sort(
        ...
        [](const JanitorFileEntry& a, const JanitorFileEntry& b) { return a.mtime < b.mtime; });
...
int ImageCache::runJanitor()
{
    ...
    const std::int64_t target = static_cast<std::int64_t>(cap * kImageCacheHysteresisRatio);
    ...
    for (std::size_t idx : victims) {
        ...
        if (nowSec - entry.mtime < kImageCacheStaleSec)
            continue;
