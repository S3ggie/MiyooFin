# Task 14 — Remove CachedLibraryView reconstruction from SQLite membership handling

**Phase:** B — ownership/modularity

## Objective

Represent current membership with domain view metadata instead of rebuilding CachedLibraryView for Anime/view classification.

## Why

Home currently reconstructs cache-shaped view objects from SQLite membership to reuse classification helpers.

## Preconditions

- Task 11 has domain results.
- Task 13 isolates snapshot compatibility.

## Allowed Files

- `src/library/LibraryQuery.hpp`/`.cpp`
- `src/ui/ShowsBrowser.hpp`
- `src/ui/screens/HomeScreen.cpp`
- `src/ui/screens/HomeScreenSync.cpp` if residual classification remains
- `tests/cases/test_catalog_parity.inc`
- `tests/cases/test_ui_foundation.inc`

## Forbidden Scope

- Do not change Anime product rules.
- Do not redesign Shows/Anime layout.
- Do not delete CachedLibraryView if compatibility still needs it.
- Do not alter artwork fetching.
...
1. Use a domain view descriptor containing id/name/collection type.
2. Anime classification consumes view metadata + MediaItem, not CachedLibraryView.
3. Remove normal SQLite-path construction of temporary CachedLibraryView.
4. Anime membership/genre retains precedence over normal Shows grouping.
5. Preserve deterministic organizational sort.

## Tests

- Anime by view-name token works.
- Anime by genre works.
- Non-Anime stays Shows.
- Series in normal+Anime membership appears once in Anime.
- No runtime membership test constructs CachedLibraryView.
...
```text
refactor(library): remove cache-shaped membership
```

Do not include any part of Task 15 in this commit.
