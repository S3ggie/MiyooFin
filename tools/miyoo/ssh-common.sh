...
MIYOO_SSH_CONNECT_TIMEOUT=${MIYOO_SSH_CONNECT_TIMEOUT:-10}
...
miyoo_ssh() {
    if [ -n "${MIYOO_SSH_CONTROL_PATH:-}" ]; then
        ssh -T -p "$MIYOO_SSH_PORT" -i "$MIYOO_SSH_KEY" \
            ...
            -o ConnectTimeout="$MIYOO_SSH_CONNECT_TIMEOUT" -o ControlPath="$MIYOO_SSH_CONTROL_PATH" "$@"
    else
        ssh -T -p "$MIYOO_SSH_PORT" -i "$MIYOO_SSH_KEY" \
            -o BatchMode=yes -o PasswordAuthentication=no \
            -o StrictHostKeyChecking=accept-new \
            -o ConnectTimeout="$MIYOO_SSH_CONNECT_TIMEOUT" "$@"
    fi
}
