...
bool JellyfinApi::getSeasons(const std::string& baseUrl, const std::string& accessToken,
                             ...
                             std::string& error, const std::atomic<bool>* cancelled)
{
    ...
    if (!response.ok()) {
        char buf[128];
        std::snprintf(buf, sizeof(buf), "Failed to fetch seasons (HTTP %ld)", response.status);
        error = buf;
        return false;
    }
...
bool JellyfinApi::getEpisodes(const std::string& baseUrl, const std::string& accessToken,
                              const std::string& userId, const std::string& deviceId,
                              ...
                              const std::atomic<bool>* cancelled)
{
    ...
    std::snprintf(urlBuf, sizeof(urlBuf),
                  ...
                  baseUrl.c_str(), seriesId.c_str(), userId.c_str(), seasonId.c_str());
    HttpResponse response;
    ...
    if (!response.ok()) {
        char buf[128];
        std::snprintf(buf, sizeof(buf), "Failed to fetch episodes (HTTP %ld)", response.status);
        error = buf;
        return false;
    }
    auto itemStrs = jsonExtractArray(response.body, "Items");
