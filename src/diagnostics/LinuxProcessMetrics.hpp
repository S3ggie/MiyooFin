...
{
  public:
    explicit LinuxProcessMetrics(std::string procRoot = "/proc", std::string storagePath = "/");

    LinuxProcessMetricsSnapshot sample(bool includeFreeStorage = true) noexcept;
    const std::string& storagePath() const noexcept
    {
    ...
    static bool parseProcIo(const std::string& contents, uint64_t& readBytes,
                            uint64_t& writeBytes) noexcept;
    static bool parseProcStatm(const std::string& contents, uint64_t pageSize,
    ...
    LinuxProcessMetricsSnapshot sampleFromProcText(const std::string& statm, const std::string& io,
                                                   uint64_t processCpuUs, uint64_t peakRssKib,
                                                   uint64_t freeStorageBytes) noexcept;

  private:
    LinuxProcessMetricsSnapshot makeSnapshot(uint64_t processCpuUs, bool peakValid,
                                             uint64_t peakRssKib, bool freeValid,
                                             uint64_t freeStorageBytes, const std::string& statm,
