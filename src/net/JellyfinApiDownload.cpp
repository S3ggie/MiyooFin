                                          ...
                                          std::string& error)
{
    ...
    const std::string url =
        baseUrl + "/Items/" + itemId + "/PlaybackInfo?UserId=" + userId + "&StartTimeTicks=0";
    ...
    if (!client.perform("POST", url, headers,
                        "{\"UserId\":\"" + userId + "\",\"StartTimeTicks\":0}", response, error)) {
        if (error.empty())
    ...
    for (const auto& obj : jsonExtractArray(response.body, "MediaSources")) {
        ...
        if (!rawSize.empty()) {
            char* end = nullptr;
            unsigned long long v = std::strtoull(rawSize.c_str(), &end, 10);
            if (end && !*end)
                s.size = v;
        }
        ...
        if (!rawRuntime.empty()) {
            char* end = nullptr;
            long long v = std::strtoll(rawRuntime.c_str(), &end, 10);
            if (end && !*end && v >= 0)
                s.runtimeTicks = v;
        }
        ...
        if (!rawBitrate.empty()) {
            char* end = nullptr;
            unsigned long long v = std::strtoull(rawBitrate.c_str(), &end, 10);
            if (end && !*end)
                s.bitrate = v;
        }
        s.videoCodec = jsonStringField(obj, "VideoCodec");
        s.audioCodec = jsonStringField(obj, "AudioCodec");
        s.width = jsonIntField(obj, "Width");
        s.height = jsonIntField(obj, "Height");
                                           ...
                                           const std::string& m, const std::string& p)
{
    return b + "/Videos/" + i +
           ...
           std::to_string(HLS_MAX_WIDTH) + "&MaxHeight=" + std::to_string(HLS_MAX_HEIGHT) +
           "&MaxFramerate=" + std::to_string(HLS_MAX_FRAMERATE) +
           "&MaxVideoBitDepth=8&VideoBitRate=" + std::to_string(HLS_VIDEO_BITRATE) +
                                    ...
                                    std::string& e, HlsFailure* failure)
{
    ...
    auto fail = [&](const char* stage) {
        HlsFailure f = classifyHlsFailure(r.status, r.transportCode);
        if (failure)
            *failure = f;
        if (r.status)
            e = std::string(stage) + " HTTP " + std::to_string(r.status);
        else if (f == HlsFailure::Timeout)
            e = "Preparing transcode timed out";
        else if (e.empty())
    ...
    std::printf("[Download] HLS master request...\n");
    {
        ...
        if (!c.perform("GET", u, buildAuthHeaders(t, d), {}, r, e) || !r.ok()) {
            std::printf("[Download] HLS master HTTP %ld\n", r.status);
            return fail("HLS master");
        }
    }
    std::printf("[Download] HLS master HTTP %ld\n", r.status);
    ...
    if (!v.empty()) {
        ...
        {
            TelemetryRequestScope request(RequestKind::HlsVariant);
            if (!c.perform("GET", u, buildAuthHeaders(t, d), {}, r, e) || !r.ok()) {
                std::printf("[Download] HLS variant HTTP %ld\n", r.status);
                return fail("HLS variant");
            }
        }
        std::printf("[Download] HLS variant HTTP %ld\n", r.status);
    }
    ...
    if (out.empty()) {
        ...
        e = "HLS playlist has no segments";
        return false;
    }
    std::printf("[Download] HLS segments=%zu\n", out.size());
    return true;
}
