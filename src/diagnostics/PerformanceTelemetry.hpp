...
class PerformanceTelemetry
{
  public:
    ...
    using ProcessMetricsHook = LinuxProcessMetricsSnapshot (*)(const LinuxProcessMetrics&,
                                                               bool) noexcept;
    struct TestHooks
    {
    ...
    bool emitSessionEvent(SessionEventKind kind, Outcome outcome, uint32_t value0,
    ...
    void setDownloadGauges(uint32_t activeDownloads, uint32_t queuedDownloads,
    ...
    void addCatalogDbRows(uint32_t inserted, uint32_t updated, uint32_t deleted) noexcept;
  ...
  private:
    ...
    void emitStateTransition(StateKind kind, uint16_t previous, uint16_t current) noexcept;
...
class PerformanceTelemetry
{
  public:
    ...
    inline bool enabledFast() const noexcept
    {
        return false;
    }
    inline void suspendSampling(bool, SamplingReason) noexcept {}
    inline uint64_t nextEphemeralId() noexcept
    {
        return 0;
    }
    inline bool emitRecord(const TelemetryRecord&) noexcept
    {
        return false;
    }
    inline bool emitSessionEvent(SessionEventKind, Outcome, uint32_t, uint64_t) noexcept
    {
        return false;
    }
    inline uint64_t droppedRecordCount() const noexcept
    {
        return 0;
    }
    inline uint64_t writerErrorCount() const noexcept
    {
        return 0;
    }
    inline uint32_t rotationCount() const noexcept
    {
        return 0;
    }
    inline uint32_t samplingLateCount() const noexcept
    {
        return 0;
    }
    inline bool serviceThreadActive() const noexcept
    {
        return false;
    }
    static ScreenId screenIdFromDiagnosticName(const char*) noexcept
    {
        return ScreenId::Other;
    }
    static TabId tabIdFromDiagnosticName(const char*) noexcept
    {
        return TabId::Other;
    }
    static ActionId actionIdFromAction(Action) noexcept
    {
