...
namespace miyoofin {
std::string LibraryCache::scopeKey(const std::string&, const std::string&)
{
    return "unused";
}
}
...
int main(int argc, char** argv)
{
    ...
    const char* bridgeBin = argc > 1 ? argv[1] : MIYOOFIN_BRIDGE_BIN;
    const int port = 24000 + (getpid() % 10000);
    std::string root = "/tmp/miyoofin-bridge-local-" + std::to_string((long long)getpid());
    DownloadStore store(root);
    DownloadItem item;
    item.itemId = "fixture";
    item.expectedSize = 11;
    item.chunkSize = 4;
    item.downloadedBytes = 11;
    item.state = DownloadState::Complete;
    CHECK(store.saveManifest("scope", item));
    std::string chunks = store.itemPath("scope", item.itemId) + "/chunks";
    CHECK(mkdir(chunks.c_str(), 0755) == 0);
    const char* parts[] = {"ABCD", "EFGH", "IJK"};
    for (int i = 0; i < 3; i++) {
        FILE* f = fopen(store.chunkPath("scope", item.itemId, i).c_str(), "wb");
        CHECK(f);
        if (f) {
            CHECK(fwrite(parts[i], 1, strlen(parts[i]), f) == strlen(parts[i]));
            fclose(f);
        }
    }
    pid_t child = fork();
    CHECK(child >= 0);
    if (child == 0) {
        execl(bridgeBin, bridgeBin, "--local-manifest",
              store.manifestPath("scope", item.itemId).c_str(), std::to_string(port).c_str(),
              (char*)nullptr);
        _exit(127);
    }
    std::string ready;
    for (int i = 0; i < 50 && ready.empty(); ++i) {
        usleep(20000);
        ready = request(port, "HEAD /stream HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n");
    }
    CHECK(!ready.empty());
    ...
    CHECK(has(inside, "206 Partial Content") && has(inside, "Content-Range: bytes 5-6/11") &&
          body(inside) == "FG");
    ...
    CHECK(has(invalid, "416 Range Not Satisfiable") && has(invalid, "Content-Range: bytes */11") &&
          body(invalid).empty());
    kill(child, SIGTERM);
    int status = 0;
    ...
    CHECK(store.ensureHlsDirectories("scope", hls.itemId));
    const char* segments[] = {"ONE", "TW"};
    for (int i = 0; i < 2; i++) {
        FILE* f = fopen(store.segmentPath("scope", hls.itemId, i).c_str(), "wb");
        CHECK(f);
        if (f) {
            CHECK(fwrite(segments[i], 1, strlen(segments[i]), f) == strlen(segments[i]));
            fclose(f);
        }
    }
    child = fork();
    ...
    if (child == 0) {
        execl(bridgeBin, bridgeBin, "--local-manifest",
              store.manifestPath("scope", hls.itemId).c_str(), std::to_string(port).c_str(),
              (char*)nullptr);
        _exit(127);
    }
    ready.clear();
    for (int i = 0; i < 50 && ready.empty(); ++i) {
        usleep(20000);
        ready = request(port, "HEAD /local.m3u8 HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n");
    }
    ...
    CHECK(has(second, "HTTP/1.1 200 OK") && has(second, "Content-Length: 2") &&
          body(second) == "TW");
    ...
    std::string traversal =
        request(port, "GET /segments/000000/../manifest.v2 HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n");
    CHECK(has(traversal, "HTTP/1.1 404 Not Found") && body(traversal).empty());
    std::string invalidSegment =
        request(port, "GET /segments/000003 HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n");
    CHECK(has(invalidSegment, "HTTP/1.1 404 Not Found") && body(invalidSegment).empty());
    kill(child, SIGTERM);
    waitpid(child, &status, 0);
    CHECK(WIFEXITED(status));
    unlink(store.segmentPath("scope", hls.itemId, 0).c_str());
    unlink(store.segmentPath("scope", hls.itemId, 1).c_str());
    unlink(store.manifestPath("scope", hls.itemId).c_str());
    rmdir((store.itemPath("scope", hls.itemId) + "/segments").c_str());
    rmdir(store.itemPath("scope", hls.itemId).c_str());
    unlink(store.chunkPath("scope", item.itemId, 0).c_str());
    unlink(store.chunkPath("scope", item.itemId, 1).c_str());
    unlink(store.chunkPath("scope", item.itemId, 2).c_str());
    rmdir(chunks.c_str());
    unlink(store.manifestPath("scope", item.itemId).c_str());
    rmdir(store.itemPath("scope", item.itemId).c_str());
    rmdir((store.scopePath("scope") + "/items").c_str());
    rmdir(store.scopePath("scope").c_str());
    rmdir(root.c_str());
    if (!fails)
        std::printf("[test] local bridge integration OK\n");
    return fails ? 1 : 0;
}
