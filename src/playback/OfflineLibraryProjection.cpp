...
OfflineLibraryProjection::OfflineLibraryProjection(const LibrarySnapshot &library, const OfflineCatalogSnapshot &catalog, const DownloadSnapshot &downloads) : m_library(library),m_catalog(catalog) {
    ...
    for(const auto &d:downloads.items) if(completeState(d.state) && d.itemType=="episode") {
        MediaItem ep; ep.id=d.itemId; ep.type="episode"; ep.title=d.title.empty()?"Episode "+std::to_string(d.episodeNumber):d.title; ep.seriesId=d.seriesId;
            ep.seriesName=d.seriesName; ep.seasonId=d.seasonId; ep.indexNumber=d.episodeNumber; ep.parentIndexNumber=d.seasonNumber; ep.playbackPositionTicks=d.playbackPositionTicks;
            ep.runTimeTicks=d.runtimeTicks;
        if(!ep.seasonId.empty()) m_episodes[ep.seasonId].push_back(ep);
        m_media[ep.id]=ep;
        if(!ep.seasonId.empty() && !m_seasons.count(ep.seasonId)) { MediaItem s; s.id=ep.seasonId;s.seriesId=ep.seriesId;s.type="season";s.indexNumber=ep.parentIndexNumber;
                s.title=d.seasonName.empty()?"Season "+std::to_string(ep.parentIndexNumber):d.seasonName;m_seasons[s.id]=s; }
...
std::vector<MediaItem> OfflineLibraryProjection::movies() const { std::vector<MediaItem> out;
        for(const auto &v:m_library.movies) for(const auto&i:v.items) if(playable(i.id)) out.push_back(i);
        unique(out);
        std::sort(out.begin(),out.end(),movieOrganizationalLess);
        return out; }
std::vector<MediaItem> OfflineLibraryProjection::episodes(const std::string &seasonId) const { std::vector<MediaItem> out; auto it=m_episodes.find(seasonId);
        if(it!=m_episodes.end()) for(const auto&i:it->second) if(playable(i.id)) out.push_back(i);
        unique(out);
        std::sort(out.begin(),out.end(),[](const MediaItem&a,const MediaItem&b){return a.indexNumber==b.indexNumber?a.title<b.title:a.indexNumber<b.indexNumber;}); return out; }
std::vector<MediaItem> OfflineLibraryProjection::seasons(const std::string &seriesId) const { std::vector<MediaItem> out;
        for(const auto &a:m_seasons) if(a.second.seriesId==seriesId&&!episodes(a.first).empty()) out.push_back(a.second);
        unique(out);
        std::sort(out.begin(),out.end(),[](const MediaItem&a,const MediaItem&b){return a.indexNumber==b.indexNumber?a.title<b.title:a.indexNumber<b.indexNumber;}); return out; }
std::vector<MediaItem> OfflineLibraryProjection::series() const { std::vector<MediaItem> out; for(const auto&a:m_series) if(!seasons(a.first).empty()) out.push_back(a.second); unique(out);
        std::sort(out.begin(),out.end(),[](const MediaItem&a,const MediaItem&b){return a.title<b.title;}); return out; }
}
