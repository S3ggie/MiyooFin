...
constexpr Profile kProfiles[] = {
    {'A', "DELETE", "FULL", "NORMAL"}, {'B', "DELETE", "EXTRA", "NORMAL"},
    {'C', "WAL", "FULL", "NORMAL"},    {'D', "WAL", "NORMAL", "NORMAL"},
    {'E', "WAL", "FULL", "EXCLUSIVE"}, {'F', "WAL", "NORMAL", "EXCLUSIVE"},
};
...
bool scalar(sqlite3* db, const std::string& sql, std::string& value, std::string& error)
{
...
bool scalarInt(sqlite3* db, const std::string& sql, long long& value, std::string& error)
{
    std::string text;
...
bool prepareSchema(sqlite3* db, std::string& error)
{
    ...
    if (!execSql(db, applicationId.c_str(), error) ||
        !execSql(db, "PRAGMA user_version = 1;", error) || !execSql(db, "COMMIT;", error)) {
        std::string ignored;
        execSql(db, "ROLLBACK;", ignored);
        return false;
    }
...
bool configure(Database& database, std::string& error)
{
    ...
    const std::string locking =
        "PRAGMA locking_mode = " + std::string(database.profile.locking) + ";";
    const std::string journal =
        "PRAGMA journal_mode = " + std::string(database.profile.journal) + ";";
    const std::string synchronous =
        "PRAGMA synchronous = " + std::string(database.profile.synchronous) + ";";
    if (!execSql(db, locking.c_str(), error) || !execSql(db, journal.c_str(), error) ||
    ...
    const std::string expectedSynchronous =
        ...
        : database.profile.synchronous == std::string("EXTRA") ? "3"
                                                               : "1";
...
bool openDatabase(const std::string& path, const Profile& profile, Database& database,
                  std::string& error)
{
    ...
    database.path = path;
    const int rc = sqlite3_open_v2(path.c_str(), &database.db,
                                   SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nullptr);
    if (rc != SQLITE_OK || !database.db) {
        error = database.db ? sqlite3_errmsg(database.db) : "sqlite open failed";
        return false;
    }
    ...
    if (!scalarInt(database.db, "PRAGMA application_id;", applicationId, error) ||
        !scalarInt(database.db, "PRAGMA user_version;", userVersion, error)) {
        return false;
    }
...
MediaItem fixtureItem(const std::string& id, const std::string& type,
                      const std::string& seriesId = {}, const std::string& seasonId = {},
                      int index = 0)
{
    MediaItem item;
    item.id = id;
...
std::vector<MediaItem> fixtureSubtree(int seriesIndex, int seasonCount, int episodesPerSeason)
{
    ...
    for (int seasonIndex = 1; seasonIndex <= seasonCount; ++seasonIndex) {
        const std::string seasonId = seriesId + "-season-" + std::to_string(seasonIndex);
        items.push_back(fixtureItem(seasonId, "season", seriesId, {}, seasonIndex));
        for (int episodeIndex = 1; episodeIndex <= episodesPerSeason; ++episodeIndex) {
            items.push_back(fixtureItem(seasonId + "-episode-" + std::to_string(episodeIndex),
...
const char* kUpsertSql = "INSERT INTO media_items("
...
bool writeItems(sqlite3* db, const std::vector<MediaItem>& items, std::size_t& rows,
                std::string& error)
{
    ...
    const bool prepared =
        sqlite3_prepare_v2(db, "DELETE FROM item_genres WHERE item_id=?1", -1, &deleteGenres,
                           nullptr) == SQLITE_OK &&
        sqlite3_prepare_v2(db,
                           "INSERT INTO item_genres(item_id, ordinal, genre) "
                           "VALUES(?1, ?2, ?3)",
                           -1, &insertGenre, nullptr) == SQLITE_OK &&
        sqlite3_prepare_v2(db, "DELETE FROM item_image_tags WHERE item_id=?1", -1, &deleteTags,
                           nullptr) == SQLITE_OK &&
        sqlite3_prepare_v2(db,
                           "INSERT INTO item_image_tags(item_id, image_type, tag) "
                           "VALUES(?1, ?2, ?3)",
                           -1, &insertTag, nullptr) == SQLITE_OK &&
        sqlite3_prepare_v2(db,
                           "SELECT ordinal, genre FROM item_genres WHERE item_id=?1 "
                           "ORDER BY ordinal",
                           -1, &selectGenres, nullptr) == SQLITE_OK &&
        sqlite3_prepare_v2(db,
                           "SELECT image_type, tag FROM item_image_tags WHERE item_id=?1 "
                           "ORDER BY image_type",
                           -1, &selectTags, nullptr) == SQLITE_OK;
    if (!prepared) {
        error = sqlite3_errmsg(db);
...
TransactionResult writeTransaction(sqlite3* db, const std::vector<MediaItem>& items)
{
...
bool deleteFixture(sqlite3* db, const std::string& id, const char* kind, std::string& error)
{
    const std::string sql =
        std::string("DELETE FROM media_items WHERE kind=") + kind + " AND id=?1;";
    sqlite3_stmt* statement = nullptr;
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &statement, nullptr) != SQLITE_OK) {
        error = sqlite3_errmsg(db);
        return false;
...
Metrics processMetrics()
{
    ...
    if (getrusage(RUSAGE_SELF, &usage) == 0) {
        metrics.cpuUs =
            static_cast<long long>(usage.ru_utime.tv_sec) * 1000000LL + usage.ru_utime.tv_usec +
...
long long fileSize(const std::string& path)
{
    ...
    {};
    return ::stat(path.c_str(), &information) == 0 ? static_cast<long long>(information.st_size)
                                                   : 0;
}

ArtifactSizes artifactSizes(const std::string& path)
{
    return {fileSize(path), fileSize(path + "-journal"), fileSize(path + "-wal"),
            fileSize(path + "-shm")};
}

void printArtifactSizes(const ArtifactSizes& sizes)
{
    std::printf("db_bytes=%lld journal_bytes=%lld wal_bytes=%lld shm_bytes=%lld", sizes.database,
                sizes.journal, sizes.wal, sizes.shm);
}

bool checkpointIfNeeded(Database& database, long long& durationUs, int& checkpointCount,
                        std::string& error)
{
    ...
    const long long start = monotonicUs();
    const int rc = sqlite3_wal_checkpoint_v2(database.db, nullptr, SQLITE_CHECKPOINT_TRUNCATE,
...
bool runWorkload(char profileId, const std::string& root, const std::string& workload,
                 int iteration, std::string& error)
{
    ...
    const std::string path = root + "/profile-" + profileId + "/" + workload + "-" +
    ...
    if (workload == "W1") {
        ...
        if (!writeTransaction(database.db, fixtureSubtree(1, 2, 20)).success ||
            ...
            !deleteFixture(database.db, "benchmark-series-1-season-1-episode-1", "4", error) ||
            !deleteFixture(database.db, "benchmark-series-1-season-2", "3", error) ||
        ...
        for (int query = 0; query < 50; ++query) {
            if (!queryHierarchy(database.db, error))
                return false;
        }
    ...
    long long rows = 0;
    if (!rowCount(database.db, rows, error))
        return false;
    ...
    std::printf("profile=%c workload=%s iteration=%d result_code=%d "
                ...
                profileId, workload.c_str(), iteration, SQLITE_OK, openUs, transaction.durationUs,
                transaction.commitUs, checkpointUs, checkpointCount, rows,
                after.cpuUs - before.cpuUs, after.rssKb, after.readBytes - before.readBytes,
                after.writeBytes - before.writeBytes);
    printArtifactSizes(artifacts);
    std::printf(" journal=%s synchronous=%s locking=%s\n", profile->journal, profile->synchronous,
                profile->locking);
    return true;
}

bool runRecovery(char profileId, const std::string& root, int iteration, std::string& error)
{
    const Profile* profile = profileFor(profileId);
    if (!profile) {
        error = "unknown profile";
    ...
    const std::string path = root + "/profile-" + profileId + "/recovery-" +
                             std::to_string(iteration) + "/catalog.sqlite3";
    Database setup;
    if (!openDatabase(path, *profile, setup, error))
        return false;
    ...
    if (child == 0) {
        ...
        if (!execSql(db, "BEGIN IMMEDIATE;", childError) ||
            !writeItems(db, fixtureSubtree(1, 4, 250), childRows, childError)) {
            _exit(112);
        }
    ...
    Database recovered;
    if (!openDatabase(path, *profile, recovered, error))
        return false;
    std::string quick;
    long long rows = 0;
    if (!scalar(recovered.db, "PRAGMA quick_check;", quick, error) || quick != "ok" ||
        !rowCount(recovered.db, rows, error))
        return false;
    sqlite3_stmt* foreignKeys = nullptr;
    const bool prepared = sqlite3_prepare_v2(recovered.db, "PRAGMA foreign_key_check;", -1,
                                             &foreignKeys, nullptr) == SQLITE_OK;
    ...
    std::printf("profile=%c workload=RECOVERY iteration=%d result_code=%d "
                ...
                profileId, iteration, SQLITE_OK, checkpointUs, checkpointCount, rows,
                quick.c_str());
    printArtifactSizes(artifacts);
    std::printf(" journal=%s synchronous=%s locking=%s\n", profile->journal, profile->synchronous,
                profile->locking);
...
bool deterministicFixtureCheck(const std::string& root, std::string& error)
{
    ...
    for (int run = 0; run < 2; ++run) {
        const std::string path =
            root + "/deterministic-" + std::to_string(run) + "/catalog.sqlite3";
        Database database;
        if (!openDatabase(path, *profile, database, error))
            return false;
...
bool catalogBenchmarkRootIsIsolated(const std::string& root)
{
    ...
    return text.find("miyoofin-sqlite-benchmark") != std::string::npos &&
           ...
           text.find("/downloads") == std::string::npos && text != "/mnt/SDCARD";
}
...
bool runCatalogBenchmarkSelfTests(std::string& error)
{
    ...
    if (catalogBenchmarkProfileSupported('X') ||
        !catalogBenchmarkRootIsIsolated("/tmp/miyoofin-sqlite-benchmark-test") ||
    ...
    for (char profileId = 'A'; profileId <= 'F'; ++profileId) {
        ...
        const std::string acceptancePath =
            root + "/profile-" + profileId + "/acceptance/catalog.sqlite3";
        Database database;
        if (!openDatabase(acceptancePath, *profile, database, error)) {
            return false;
        }
        if ((profileId == 'E' || profileId == 'F') && fileSize(acceptancePath + "-shm") != 0) {
        ...
        if (profileId == 'C') {
            ...
            int checkpointCount = 0;
            if (!transaction.success ||
                !checkpointIfNeeded(database, checkpointUs, checkpointCount, error) ||
                checkpointCount != 1) {
                if (error.empty())
                    error = "WAL checkpoint fixture failed";
                return false;
            }
        }
    }
    if (!deterministicFixtureCheck(root, error))
        return false;
...
int runCatalogJournalBenchmark(int argc, char** argv)
{
    ...
    const char* workloads[] = {"W1", "W2", "W3", "W4-1", "W4-5", "W4-20", "W5", "W6", "W7"};
    std::string error;
