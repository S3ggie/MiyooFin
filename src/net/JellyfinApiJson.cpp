...
// -------------------------------------------------------------------
static int decodeUnicodeEscape(const std::string& s, size_t pos, std::string& out)
{
    // pos points at 'u'
    if (pos + 4 >= s.size())
    ...
    if (cp >= 0xD800 && cp <= 0xDBFF) {
        ...
        if (pos + 10 < s.size() && s[pos + 5] == '\\' && s[pos + 6] == 'u') {
            ...
            if (lh0 >= 0 && lh1 >= 0 && lh2 >= 0 && lh3 >= 0) {
                uint32_t lo = (uint32_t(lh0) << 12) | (uint32_t(lh1) << 8) | (uint32_t(lh2) << 4) |
                              uint32_t(lh3);
    ...
    while (pos < s.size()) {
        if (s[pos] == '\\' && pos + 1 < s.size()) {
            ...
            case 'f':
                ...
                break;
            case 'u':
                pos += decodeUnicodeEscape(s, pos, out) - 1;
                break;
            default:
                out += s[pos];
                break;
...
// -------------------------------------------------------------------
std::string JellyfinApi::extractNestedString(const std::string& json, const std::string& parent,
                                             const std::string& child)
{
    ...
    while (afterChild < json.size() &&
        ...
        afterChild++;
    if (afterChild >= json.size() || json[afterChild] != '\"')
        return {};
    afterChild++;
    std::string val;
    while (afterChild < json.size() && json[afterChild] != '\"') {
        if (json[afterChild] == '\\' && afterChild + 1 < json.size()) {
            afterChild++;
            if (json[afterChild] == 'n')
                val += '\n';
            else if (json[afterChild] == 'r')
                val += '\r';
            else if (json[afterChild] == 't')
                val += '\t';
            else if (json[afterChild] == '\\')
                val += '\\';
            else if (json[afterChild] == '\"')
                val += '\"';
...
std::string JellyfinApi::jsonRawValue(const std::string& json, const std::string& key)
{
    std::string search = "\"" + key + "\"";
    auto pos = json.find(search);
    if (pos == std::string::npos)
...
std::string JellyfinApi::jsonStringField(const std::string& obj, const std::string& key)
{
...
float JellyfinApi::jsonFloatField(const std::string& obj, const std::string& key)
{
    std::string v = jsonRawValue(obj, key);
    if (v.empty() || v == "null")
        return 0.0f;
...
bool JellyfinApi::jsonBoolField(const std::string& obj, const std::string& key)
{
    return jsonRawValue(obj, key) == "true";
}

std::vector<std::string> JellyfinApi::splitJsonArrayContent(const std::string& content)
{
            ...
            if (ins) {
                ...
                if (c == '"')
                    ...
                    while (b > a && (e[b - 1] == ' ' || e[b - 1] == '\n' || e[b - 1] == '\r' ||
                        ...
                        b--;
                    if (a < b)
                        result.push_back(e.substr(a, b - a));
                    es = i + 1;
                }
            }
        } else {
...
std::vector<std::string> JellyfinApi::jsonExtractArray(const std::string& json,
...
MediaItem JellyfinApi::jsonToMediaItem(const std::string& obj)
{
    ...
    item.parentIndexNumber = jsonIntField(obj, "ParentIndexNumber");
    {
        ...
        if (!rt.empty() && rt != "null") {
            try {
                item.runTimeTicks = std::stoll(rt);
            } catch (...) {
                item.runTimeTicks = 0;
            }
        }
    ...
    if (!ur.empty() && ur[0] == '{') {
        ...
        if (!position.empty() && position != "null") {
            try {
                item.playbackPositionTicks = std::stoll(position);
            } catch (...) {
    ...
    if (!ir.empty() && ir[0] == '{') {
        ...
        while (p < ir.size()) {
            ...
            while (p < ir.size() && ir[p] != '"')
                p++;
            std::string ik = ir.substr(ks, p - ks);
            p++;
            while (p < ir.size() && ir[p] != ':')
                p++;
            if (p >= ir.size())
            ...
            if (p < ir.size() && ir[p] == '"') {
                ...
                while (p < ir.size() && ir[p] != '"')
                    p++;
                item.imageTags[ik] = ir.substr(vs, p - vs);
                p++;
            }
            while (p < ir.size() && ir[p] != ',' && ir[p] != '}')
                p++;
            if (p < ir.size() && ir[p] == ',')
                p++;
        }
