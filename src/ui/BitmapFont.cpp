...
static int utf8SequenceBytes(const unsigned char* p, const unsigned char* end)
{
    if (p >= end || *p == 0)
        return 0;
...
std::string BitmapFont::truncateUtf8(const std::string& text, int maxGlyphs)
{
    ...
    return std::string(reinterpret_cast<const char*>(bytes), cut - bytes) + suffix;
}

void BitmapFont::drawChar(SDL_Surface* surface, int x, int y, unsigned char ch, Uint8 fgR,
                          Uint8 fgG, Uint8 fgB, Uint8 bgR, Uint8 bgG, Uint8 bgB)
{
    if (!surface)
        return;
    ...
    for (int row = 0; row < maxY; ++row) {
        ...
        Uint32* pixel =
            (Uint32*)((Uint8*)surface->pixels + (y + row) * surface->pitch + x * sizeof(Uint32));
...
void BitmapFont::drawString(SDL_Surface* surface, int x, int y, const char* text, Uint8 fgR,
                            Uint8 fgG, Uint8 fgB, Uint8 bgR, Uint8 bgG, Uint8 bgB, int wrapCols)
{
    if (!text)
        return;
...
void BitmapFont::drawStringScaled(SDL_Surface* surface, int x, int y, const char* text, int scale,
    ...
    for (const unsigned char* p = reinterpret_cast<const unsigned char*>(text); *p; ++p) {
        ...
        for (int row = 0; row < GLYPH_H; ++row) {
            for (int col = 0; col < GLYPH_W; ++col) {
                ...
                fillRect(surface, cursorX + col * scale, y + row * scale, scale, scale,
                         set ? fgR : bgR, set ? fgG : bgG, set ? fgB : bgB, 255);
            }
        }
...
void BitmapFont::fillRect(SDL_Surface* surface, int x, int y, int w, int h, Uint8 r, Uint8 g,
                          Uint8 b, Uint8 a)
{
    if (!surface)
        return;
