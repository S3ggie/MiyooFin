...
inline bool estimateHlsBytes(std::int64_t runtimeTicks, std::uint64_t& bytes)
{
    ...
    if (seconds > (std::numeric_limits<std::uint64_t>::max)() / bytesPerSecond)
        return false;
    bytes = seconds * bytesPerSecond;
    const std::uint64_t partial =
        (remainder * HLS_TOTAL_BITRATE + HLS_TICKS_PER_SECOND * 8 - 1) / (HLS_TICKS_PER_SECOND * 8);
    if (bytes > (std::numeric_limits<std::uint64_t>::max)() - partial)
        return false;
    bytes += partial;
    const std::uint64_t overhead = bytes / HLS_OVERHEAD_DIVISOR;
    if (bytes > (std::numeric_limits<std::uint64_t>::max)() - overhead ||
        bytes + overhead > (std::numeric_limits<std::uint64_t>::max)() - HLS_FIXED_OVERHEAD_BYTES)
        return false;
