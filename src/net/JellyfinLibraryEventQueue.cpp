...
JellyfinLibraryEventQueue::JellyfinLibraryEventQueue(std::size_t capacity)
...
bool JellyfinLibraryEventQueue::push(const JellyfinLibraryChangeBatch& batch)
{
    ...
    bool accepted = true;
    const auto merge = [&](const std::vector<std::string>& ids, unsigned flag) {
...
bool JellyfinLibraryEventQueue::pop(JellyfinLibraryChangeBatch& batch)
{
    ...
    for (const auto& entry : m_ids) {
        if (entry.second & 1u)
            batch.itemsAdded.push_back(entry.first);
        if (entry.second & 2u)
            batch.itemsRemoved.push_back(entry.first);
        if (entry.second & 4u)
            batch.itemsUpdated.push_back(entry.first);
    }
    m_ids.clear();
    m_overflowed = false;
    return true;
