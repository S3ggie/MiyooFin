...
inline uint32_t clampToUint32(uint64_t value) noexcept
{
    return value > std::numeric_limits<uint32_t>::max() ? std::numeric_limits<uint32_t>::max()
                                                        : static_cast<uint32_t>(value);
}
inline void updateMaximum(std::atomic<uint32_t>& maximum, uint64_t value) noexcept
{
    const uint32_t clamped = clampToUint32(value);
    uint32_t observed = maximum.load(std::memory_order_relaxed);
    while (clamped > observed &&
           !maximum.compare_exchange_weak(observed, clamped, std::memory_order_relaxed,
                                          std::memory_order_relaxed)) {
    }
}
inline LinuxProcessMetricsSnapshot sampleProcessMetrics(LinuxProcessMetrics& metrics,
