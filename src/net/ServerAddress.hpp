...
inline bool isObviousLanServerUrl(const std::string& url)
{
    ...
    if (colon != std::string::npos) {
        if (authority.find(':', colon + 1) != std::string::npos || colon == 0 ||
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if ((i < 3 && end == std::string::npos) || (i == 3 && end != std::string::npos))
            return false;
        const std::string part =
            host.substr(start, (end == std::string::npos ? host.size() : end) - start);
