...
CatalogMigrationTestPaths catalogMigrationTestPaths(const std::string& url, const std::string& user)
{
    ...
    CHECK(paths.finalPath == miyoofin::catalog::catalogPath("cache", paths.scope));
    CHECK(paths.migratingPath == miyoofin::catalog::migratingCatalogPath("cache", paths.scope));
...
{
    const std::string files[] = {paths.finalPath,
                                 paths.finalPath + "-journal",
                                 paths.finalPath + "-wal",
                                 paths.finalPath + "-shm",
                                 paths.migratingPath,
                                 paths.migratingPath + "-journal",
                                 paths.migratingPath + "-wal",
                                 paths.migratingPath + "-shm",
                                 paths.legacyPath};
