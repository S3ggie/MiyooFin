...
CatalogDbEnqueueResult CatalogDb::enqueueNoopForTest(CatalogDbPriority priority,
...
{
    {
        ...
        m_queues[priorityIndex(priority)].push_back({priority, metadata, telemetryNowIfEnabled()});
        ++m_pendingJobs;
...
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return {m_connectionOpen, m_connectionWorkerOwned, m_preparedStatementCount};
}
...
CatalogDbTestResult CatalogDb::setSchemaMetadataForTest(std::int64_t applicationId,
                                                        std::int64_t userVersion)
{
    return runTestCommand(kSetSchemaMetadataOperation,
                          std::to_string(applicationId) + ":" + std::to_string(userVersion));
}
...
CatalogDbTestResult CatalogDb::runMediaPageQueryPlanForTest(const std::string& type,
                                                            int alphabetLetter)
{
...
CatalogDbEnqueueResult CatalogDb::enqueueScopedNoopForTest(CatalogDbPriority priority)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_stopping) {
        performanceTelemetry().addCatalogDbEnqueueRejected();
...
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return scopeEpoch != 0 && scopeEpoch == m_requestedEpoch && m_scopeConfigured && m_scopeReady;
}
...
void CatalogDb::processTestCommand(const std::shared_ptr<TestCommand>& command)
{
    ...
    auto prepareNamed = [&](const char* name, const char* sql, sqlite3_stmt*& statement) {
    ...
    case kDiagnosticsOperation:
        if (!scalar(m_db, "PRAGMA foreign_keys;", result.foreignKeys, result.message) ||
            !scalar(m_db, "PRAGMA trusted_schema;", result.trustedSchema, result.message) ||
            !scalar(m_db, "PRAGMA journal_mode;", result.journalMode, result.message) ||
            !scalar(m_db, "PRAGMA synchronous;", result.synchronous, result.message) ||
            !scalar(m_db, "PRAGMA locking_mode;", result.lockingMode, result.message)) {
            result.error = CatalogDbErrorCategory::SqliteError;
            break;
        }
        result.success = result.foreignKeys == "1" && result.trustedSchema == "0" &&
                         result.journalMode == "delete" && result.synchronous == "2" &&
    ...
    case kSchemaDiagnosticsOperation: {
        ...
        if (!collectObjects(m_db, objects, result.message) ||
            !scalarInt(m_db, "PRAGMA application_id;", result.applicationId, result.message) ||
            !scalarInt(m_db, "PRAGMA user_version;", result.userVersion, result.message) ||
            !scalar(m_db,
                    "SELECT singleton_id FROM sync_state "
                    "WHERE singleton_id=1;",
                    result.sentinel, result.message) ||
            !schemaConstraints(m_db, result.foreignKeyCascade, result.checkConstraints,
                               result.message)) {
            result.error = CatalogDbErrorCategory::SqliteError;
            break;
        }
        ...
        result.success = result.exactSchema && result.foreignKeyCascade &&
                         result.checkConstraints && result.singletonSeeded &&
                         result.applicationId == kCatalogApplicationId && result.userVersion == 3;
        if (!result.success) {
            result.error = CatalogDbErrorCategory::ConfigurationFailed;
    ...
    case kSqlErrorOperation: {
        ...
        const int rc = sqlite3_prepare_v2(m_db, "SELECT * FROM catalog_task05_missing_table", -1,
                                          &statement, nullptr);
        int stepRc = rc;
        if (rc == SQLITE_OK) {
        ...
        const bool sqliteFailed =
            stepRc != SQLITE_OK && stepRc != SQLITE_ROW && stepRc != SQLITE_DONE;
        result.success = false;
        result.error = sqliteFailed ? CatalogDbErrorCategory::SqliteError
                                    : CatalogDbErrorCategory::ConfigurationFailed;
        result.message =
            sqliteFailed ? sqlite3_errmsg(m_db) : "expected SQLite error was not returned";
        break;
    }
    ...
    case kMediaItemCodecOperation: {
        ...
        if (!prepareNamed("media_item_scalar_insert",
        ...
        auto roundTrip = [&](const MediaItem& expected, bool nullableRelationships) {
            ...
            if (nullableRelationships && (sqlite3_bind_null(insert, 15) != SQLITE_OK ||
            ...
            if (sqlite3_bind_text(select, 1, expected.id.c_str(), -1, SQLITE_TRANSIENT) !=
                SQLITE_OK) {
        ...
        result.codecPopulatedKinds = roundTrip(series, false) && roundTrip(season, false) &&
                                     roundTrip(episode, false) && roundTrip(movie, false);

        MediaItem defaults;
        defaults.id = "__task08_defaults__";
        defaults.type = "movie";
        result.codecDefaults = roundTrip(defaults, true);
        result.codecNullableRelationships =
            result.codecDefaults && defaults.seriesId.empty() && defaults.seasonId.empty();
        ...
        const bool freshDefaults =
            scalarInt(m_db, "SELECT art_r FROM media_items WHERE id='__task08_defaults__';", artR,
                      result.message) &&
            artR == 128 &&
            scalarInt(m_db, "SELECT art_g FROM media_items WHERE id='__task08_defaults__';", artG,
                      result.message) &&
            artG == 128 &&
            scalarInt(m_db, "SELECT art_b FROM media_items WHERE id='__task08_defaults__';", artB,
                      result.message) &&
            artB == 128 &&
            exec(m_db,
                 "UPDATE media_items SET art_r=7, art_g=19, art_b=231 "
                 "WHERE id='__task08_defaults__';",
                 result.message);
        MediaItem legacy = defaults;
        const bool legacyUpsert =
            freshDefaults && roundTrip(legacy, true) &&
            scalarInt(m_db, "SELECT art_r FROM media_items WHERE id='__task08_defaults__';", artR,
                      result.message) &&
            artR == 7 &&
            scalarInt(m_db, "SELECT art_g FROM media_items WHERE id='__task08_defaults__';", artG,
        ...
        result.codecInvalidKind = !bindMediaItemScalars(insert, invalidKind, invalidError) &&
        ...
        result.codecMissingId = !bindMediaItemScalars(insert, missingId, missingError) &&
                                missingError == MediaItemSqlError::MissingId;
        reset(insert);

        exec(m_db, "DELETE FROM media_items WHERE id LIKE '__task08_%';", result.message);
        result.success = result.codecPopulatedKinds && result.codecDefaults &&
                         result.codecBoundaries && result.codecLegacyArtworkIgnored &&
                         result.codecInvalidKind && result.codecMissingId &&
                         result.codecNullableRelationships;
        if (!result.success) {
            result.error = CatalogDbErrorCategory::ConfigurationFailed;
    ...
    case kMediaItemCollectionsOperation: {
        ...
        if (!prepareNamed("media_item_scalar_insert",
            ...
            !prepareNamed(
                ...
                "runtime_ticks, series_name, series_id, season_id FROM media_items WHERE id=?1",
                select) ||
            !prepareNamed("media_item_genres_delete", "DELETE FROM item_genres WHERE item_id=?1",
                          deleteGenres) ||
            !prepareNamed("media_item_genre_insert",
                          "INSERT INTO item_genres(item_id, ordinal, genre) "
                          "VALUES(?1, ?2, ?3)",
                          insertGenre) ||
            !prepareNamed("media_item_image_tags_delete",
                          "DELETE FROM item_image_tags WHERE item_id=?1", deleteImageTags) ||
            !prepareNamed("media_item_image_tag_insert",
                          "INSERT INTO item_image_tags(item_id, image_type, tag) "
                          "VALUES(?1, ?2, ?3)",
                          insertImageTag) ||
            !prepareNamed("media_item_genres_select",
                          "SELECT ordinal, genre FROM item_genres WHERE item_id=?1 "
                          "ORDER BY ordinal",
                          selectGenres) ||
            !prepareNamed("media_item_image_tags_select",
                          "SELECT image_type, tag FROM item_image_tags WHERE item_id=?1 "
                          "ORDER BY image_type",
                          selectImageTags) ||
            !prepareNamed("media_item_delete", "DELETE FROM media_items WHERE id=?1", deleteItem)) {
            break;
        }

        const MediaItemCollectionStatements collections{deleteGenres,    insertGenre,
                                                        deleteImageTags, insertImageTag,
        ...
        auto insertAndRead = [&](const MediaItem& expected) {
            ...
            const bool readOk = readMediaItemScalars(select, actual, readError);
            reset(select);
            if (!readOk || !readMediaItemCollections(collections, actual, readError)) {
        ...
        item.imageTags = {
            {"Backdrop", "backdrop-tag"}, {"Primary", "primary-tag"}, {"Thumb", "thumb-tag"}};
        ...
        updated.imageTags = {{"Logo", "logo-new"}, {"Primary", "primary-new"}};
        result.collectionsUpdateRemoval = insertAndRead(updated);
        if (result.collectionsUpdateRemoval) {
            result.collectionsUpdateRemoval =
                updated.genres.size() == 1 && updated.genre == updated.genres.front();
        }
        ...
        if (!exec(m_db, "BEGIN IMMEDIATE;", result.message)) {
            ...
            if (!bindMediaItemScalars(insert, emptyTags, error) ||
                ...
                reset(insert);
                if (!replaceMediaItemCollections(collections, emptyTags, error)) {
                    result.message = "MediaItem collection replacement failed (empty)";
                    rollback();
                    result.collectionsEmptyTagPreserve = false;
                    ...
                    reset(select);
                    const bool bound = sqlite3_bind_text(select, 1, emptyTags.id.c_str(), -1,
                                                         SQLITE_TRANSIENT) == SQLITE_OK;
                    const bool row = bound && sqlite3_step(select) == SQLITE_ROW;
                    MediaItem readBack;
                    MediaItemSqlError readErr = MediaItemSqlError::None;
                    const bool readOk = row && readMediaItemScalars(select, readBack, readErr);
                    const bool collOk =
                        readOk && readMediaItemCollections(collections, readBack, readErr);
                    reset(select);
                    ...
                    result.collectionsEmptyTagPreserve =
                        collOk && readBack.imageTags.size() == 2 &&
                        ...
                        readBack.genres.size() == 1 && readBack.genres[0] == "Updated";
                }
            }
        }

        result.collectionsParity = result.collectionsZero && result.collectionsMultiple &&
                                   result.collectionsUpdateRemoval &&
                                   result.collectionsEmptyTagPreserve;
        ...
        if (!exec(m_db, "BEGIN IMMEDIATE;", result.message)) {
            ...
            reset(deleteItem);
            const bool deleteOk = sqlite3_bind_text(deleteItem, 1, item.id.c_str(), -1,
                                                    SQLITE_TRANSIENT) == SQLITE_OK &&
            ...
            if (!deleteOk) {
                ...
                deleted.id = item.id;
                MediaItemSqlError readError = MediaItemSqlError::None;
                const bool childrenEmpty =
                    readMediaItemCollections(collections, deleted, readError) &&
                    deleted.genres.empty() && deleted.genre.empty() && deleted.imageTags.empty();
                std::int64_t remaining = 1;
                const bool rowGone = scalarInt(m_db,
        ...
        result.success = result.collectionsParity && result.collectionsDeleteCascade;
        if (!result.success) {
            result.error = CatalogDbErrorCategory::ConfigurationFailed;
            if (result.message.empty()) {
    ...
    case kSeedHierarchyQueryOperation: {
        if (!exec(m_db, "BEGIN IMMEDIATE;", result.message) ||
            !exec(m_db, "DELETE FROM media_items WHERE id LIKE '__task10_%';", result.message) ||
            !exec(m_db,
        ...
        for (int index = 0; index < 200; ++index) {
            const std::string statement = "INSERT INTO media_items(id, kind, title, series_id, "
                                          "index_number) VALUES('__task10_other_season_" +
                                          std::to_string(index) +
                                          "', 3, 'Other', "
                                          "'__task10_other_series__', " +
                                          std::to_string(index) + ");";
        ...
        auto planUsesIndex = [&](const char* sql, const char* indexName) {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
                ...
                if (detail && std::string(reinterpret_cast<const char*>(detail)).find(indexName) !=
                                  std::string::npos) {
                    found = true;
                }
            }
        ...
        result.hierarchyIndexes = planUsesIndex("EXPLAIN QUERY PLAN SELECT id FROM media_items "
                                                "WHERE series_id='__task10_series__' AND kind=3 "
                                                "ORDER BY index_number, title, id;",
                                                "idx_media_series_kind_order") &&
                                  planUsesIndex("EXPLAIN QUERY PLAN SELECT id FROM media_items "
    ...
    case kMediaPageQueryPlanOperation: {
        ...
        std::string sql = "EXPLAIN QUERY PLAN SELECT id FROM media_items INDEXED BY " +
                          ("idx_media_" + type + "_sort") + " WHERE kind=" + std::to_string(kind) +
        ...
        if (sqlite3_prepare_v2(m_db, sql.c_str(), -1, &plan, nullptr) != SQLITE_OK) {
            result.error = CatalogDbErrorCategory::SqliteError;
            result.message = sqlite3_errmsg(m_db);
            break;
        }
        ...
        result.success = result.mediaPageUsesSortIndex && result.mediaPageAvoidsTempSort;
        if (!result.success) {
            result.error = CatalogDbErrorCategory::ConfigurationFailed;
    ...
    case kWriteSentinelOperation: {
        if (!exec(m_db, "CREATE TABLE IF NOT EXISTS catalog_task05_sentinel(value TEXT NOT NULL)",
                  result.message) ||
            !exec(m_db, "DELETE FROM catalog_task05_sentinel", result.message)) {
            result.error = CatalogDbErrorCategory::SqliteError;
            break;
        }
        sqlite3_stmt* statement = nullptr;
        if (!prepareNamed("sentinel_insert",
                          "INSERT INTO catalog_task05_sentinel(value) VALUES(?1)", statement)) {
            break;
        }
        sqlite3_bind_text(statement, 1, command->value.c_str(), -1, SQLITE_TRANSIENT);
    ...
    case kWriteSchemaMarkerOperation: {
        sqlite3_stmt* statement = nullptr;
        if (!prepareNamed("schema_marker_write",
    ...
    case kReadSchemaMarkerOperation: {
        sqlite3_stmt* statement = nullptr;
        if (!prepareNamed("schema_marker_read",
    ...
    case kSetSchemaMetadataOperation: {
        ...
        const std::string applicationIdPragma = "PRAGMA application_id = " + applicationId + ";";
        const std::string userVersionPragma = "PRAGMA user_version = " + userVersion + ";";
    ...
    case kMigrationRollbackOperation: {
        ...
        const int failedMigration =
            began ? execResult(m_db,
                               "CREATE TABLE catalog_task07_rollback_probe(value TEXT);"
                               "INSERT INTO catalog_task07_missing(value) VALUES(1);",
                               migrationError)
                  : SQLITE_ERROR;
        std::string rollbackError;
        const bool rolledBack = began && exec(m_db, "ROLLBACK;", rollbackError);
        std::int64_t probeCount = 1;
        const bool inspected = scalarInt(m_db,
                                         "SELECT COUNT(*) FROM sqlite_master WHERE name="
                                         "'catalog_task07_rollback_probe';",
                                         probeCount, result.message);
        sqlite3_stmt* probe = nullptr;
        const int prepareRc = sqlite3_prepare_v2(m_db, "SELECT 1", -1, &probe, nullptr);
        ...
        result.success = failedMigration != SQLITE_OK && rolledBack && inspected &&
                         probeCount == 0 && result.statementReused;
        if (!result.success) {
            result.error = CatalogDbErrorCategory::ConfigurationFailed;
            if (result.message.empty()) {
                result.message = migrationError.empty() ? rollbackError : migrationError;
            }
        }
        break;
    }
    ...
    case kReadSentinelOperation: {
        sqlite3_stmt* statement = nullptr;
        if (!prepareNamed("sentinel_read", "SELECT value FROM catalog_task05_sentinel LIMIT 1",
                          statement)) {
            break;
        }
        const int rc = sqlite3_step(statement);
...
std::future<CatalogDbHierarchyWriteResult> CatalogDb::upsertSeriesHierarchyForTest(
    ...
    std::int64_t refreshMs, int failAfterRows, int cancelAfterRows)
{
    return enqueueHierarchyWrite(series, seasons, episodesBySeason, generation, refreshMs, true,
                                 false, {},
                                 CatalogDbFailureSpec{failAfterRows, cancelAfterRows, -1});
}

std::future<CatalogDbReconcileResult>
CatalogDb::reconcileSeriesForTest(const std::vector<MediaItem>& series, bool authoritative,
                                  int failAfterRows)
{
    return enqueueReconcile(series, authoritative, {}, CatalogDbFailureSpec{failAfterRows, -1, -1});
}

std::future<CatalogDbMediaPageUpsertResult>
CatalogDb::upsertMediaPageForTest(const CatalogDbMediaPageWrite& page, int failAfterRows,
                                  const CatalogDbJobMetadata& metadata)
{
    return enqueueMediaPageUpsert(page, metadata, CatalogDbFailureSpec{failAfterRows, -1, -1});
}
