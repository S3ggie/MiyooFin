...
namespace miyoofin {
namespace {
constexpr uint32_t VERSION=1, MAX_STRING=1024*1024, MAX_ITEMS=100000, MAX_VIEWS=1024;
...
bool LibraryCache::save(const std::string&path,const LibrarySnapshot&s,std::string*e){auto slash=path.find_last_of('/');if(slash!=std::string::npos&&!mkdirs(path.substr(0,slash))){if(e)*e="mkdir failed";return false;}std::vector<unsigned char>b={'M','F','L','C'};put32(b,VERSION);pv(b,s.movies);pv(b,s.shows);std::string tmp=path+".tmp";FILE*f=std::fopen(tmp.c_str(),"wb");if(!f){if(e)*e="open failed";return false;}bool ok=std::fwrite(b.data(),1,b.size(),f)==b.size(); if(std::fclose(f)!=0)ok=false; if(!ok||std::rename(tmp.c_str(),path.c_str())!=0){std::remove(tmp.c_str());if(e)*e="write/rename failed";return false;}return true;}
bool LibraryCache::load(const std::string&path,LibrarySnapshot&o,std::string*e){FILE*f=std::fopen(path.c_str(),"rb");if(!f){if(e)*e="not found";return false;}std::fseek(f,0,SEEK_END);long z=std::ftell(f);std::fseek(f,0,SEEK_SET);if(z<8||z>128*1024*1024){std::fclose(f);if(e)*e="invalid size";return false;}std::vector<unsigned char>b((size_t)z);bool ok=std::fread(b.data(),1,b.size(),f)==b.size();std::fclose(f);size_t p=0;uint32_t v;LibrarySnapshot t;if(!ok||b[0]!='M'||b[1]!='F'||b[2]!='L'||b[3]!='C'||(p=4,!get32(b,p,v))||v!=VERSION||!gv(b,p,t.movies)||!gv(b,p,t.shows)||p!=b.size()){if(e)*e="invalid cache";return false;}o=std::move(t);return true;}
