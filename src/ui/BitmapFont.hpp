...
class BitmapFont
{
  public:
    ...
    static void drawChar(SDL_Surface* surface, int x, int y, unsigned char ch, Uint8 fgR, Uint8 fgG,
                         Uint8 fgB, Uint8 bgR, Uint8 bgG, Uint8 bgB);

    /// Draw a null-terminated UTF-8 string.
    /// @param x  left edge in pixels
    /// @param y  top edge in pixels
    /// @param wrapCols  if > 0, wrap at this column (character count)
    static void drawString(SDL_Surface* surface, int x, int y, const char* text, Uint8 fgR,
                           Uint8 fgG, Uint8 fgB, Uint8 bgR, Uint8 bgG, Uint8 bgB, int wrapCols = 0);
    ...
    static void drawRect(SDL_Surface* surface, int x, int y, int w, int h, Uint8 r, Uint8 g,
                         Uint8 b);

    /// Fill a rectangle on the surface.
    static void fillRect(SDL_Surface* surface, int x, int y, int w, int h, Uint8 r, Uint8 g,
                         Uint8 b, Uint8 a = 255);
