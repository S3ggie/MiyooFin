...
class SeriesScreen : public Screen
{
  public:
    SeriesScreen(const Session& session, const MediaItem& series,
                 std::shared_ptr<DownloadManager> downloads = {}, bool networkOffline = false,
    ...
    bool diagnosticSeasonsReady() const
    {
        return !m_seasons.empty();
    }
    const std::vector<MediaItem>& diagnosticSeasons() const
    {
        return m_seasons;
    }
    // Shared by the async completion path and focused no-network tests.
    static std::vector<MediaItem>
    replaceSeasonsKeepingSelection(const std::vector<MediaItem>& current,
  ...
  private:
    ...
    void queueArtwork(const std::string& key, const MediaItem& item, int width, int height,
                      bool series);
    ...
    static PreparedArtwork prepareArtworkSurface(const DecodedImage& image, int boxX, int boxY,
                                                 int boxW, int boxH);
    static void freePreparedArtwork(PreparedArtwork& artwork);
    void drawSeasonPoster(SDL_Surface* fb, int x, int y, int w, int h, const MediaItem& season,
                          bool selected, const PreparedArtwork* artwork);
    ...
    bool m_fetchDone = false, m_fetchOk = false, m_cachedSeasonsDone = false;
    std::vector<MediaItem> m_fetchSeasons, m_cachedSeasons;
    ...
    std::thread m_artworkThread;
    std::mutex m_artworkMutex;
    std::condition_variable m_artworkCv;
    std::vector<ArtworkJob> m_artworkJobs;
    std::map<std::string, DecodedImage> m_artworkCompleted;
    bool m_artworkStop = false;
    std::atomic<bool> m_artworkCancelled{false};
