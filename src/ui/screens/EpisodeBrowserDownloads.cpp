...
void EpisodeBrowserScreen::handleDownloadButtonAction()
{
    if (m_confirmDownload) {
        ...
        m_confirmDownload = false;
    } else if (m_downloads && m_selectedEpisode >= 0 &&
               m_selectedEpisode < (int)m_episodes.size()) {
        bool season = m_actionBtn == ActionButton::DownloadSeason;
        if (m_planId && m_planIsSeason == season &&
            m_downloads->planSnapshot(m_planId).state == DownloadPlanState::Ready)
            m_confirmDownload = true;
        else {
            m_planIsSeason = season;
            m_planId = m_downloads->requestPlan(
                season ? m_episodes : std::vector<MediaItem>{m_episodes[m_selectedEpisode]});
        }
    }
}
