...
{
    switch (scancode) {
    case static_cast<SDL_Scancode>(82):
        return 0;
    case static_cast<SDL_Scancode>(81):
        return 1;
    case static_cast<SDL_Scancode>(80):
        return 2;
    case static_cast<SDL_Scancode>(79):
        return 3;
    default:
        break;
    }
...
InputManager::InputManager() : m_joystickIndex(-1)
{
    const char* desktopInput = std::getenv("MIYOOFIN_DESKTOP_INPUT");
    m_desktopInput = desktopInput && desktopInput[0] != '\0' && desktopInput[0] != '0';
...
std::vector<Action> InputManager::poll()
{
    ...
    while (SDL_PollEvent(&ev)) {
        ...
        case SDL_KEYUP: {
            ...
            if (repeatIndex >= 0) {
                ...
                if (down) {
                    if (ev.key.repeat == 0 &&
                        beginDpadPress(m_dpadRepeatStates[repeatIndex], action, SDL_GetTicks()))
            ...
            if (down) {
                if (m_desktopInput && (kc == SDLK_RETURN || kc == SDLK_KP_ENTER)) {
                    action = Action::Confirm;
                } else if (m_desktopInput && (kc == SDLK_BACKSPACE || kc == SDLK_ESCAPE)) {
                    action = Action::Back;
                } else {
                    switch (sc) {
                    case 44:
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            addRawEvent(ev.type, true, 0, SDL_SCANCODE_UNKNOWN, ev.button.button, Action::None);
            break;
    ...
    for (auto& state : m_dpadRepeatStates) {
        if (takeDpadRepeat(state, now))
            actions.push_back(state.action);
    }

    return actions;
}

void InputManager::addRawEvent(Uint32 type, bool isDown, SDL_Keycode kc, SDL_Scancode sc, Uint8 btn,
                               Action action)
{
    RawEvent re;
    re.timestamp = SDL_GetTicks();
