...
void DownloadManager::requestReconcile()
{
    {
...
void DownloadManager::reconciler()
{
    for (;;) {
        Session session;
        std::string scope;
        std::uint64_t generation = 0;
        bool startupReconcile = false;
        std::vector<DownloadItem> items;
        {
            std::unique_lock<std::mutex> l(m_mutex);
            m_reconcileWake.wait(l, [&] { return m_stop || m_reconcileRequested; });
            if (m_stop)
                return;
            m_reconcileWake.wait_for(l, std::chrono::seconds(3), [&] { return m_stop; });
            if (m_stop)
                return;
            m_reconcileRequested = false;
            startupReconcile = m_startupReconcile;
            m_startupReconcile = false;
            performanceTelemetry().setWorkerQueueDepth(WorkerId::DownloadReconcile, 0);
            performanceTelemetry().setWorkerActive(WorkerId::DownloadReconcile, true);
            session = m_session;
            scope = m_scope;
        ...
        for (const auto& old : items) {
            if (!session.valid())
                break;
            if (reconcileShouldSkip(old, startupReconcile, nowMs))
                continue;
            ...
            if (RouteRequest(session).run(
                    [&](const std::string& base) {
                        return JellyfinApi::getDownloadMediaSources(
                            base, session.accessToken, session.userId, session.deviceId, old.itemId,
                            sources, error);
                    },
                    error) &&
                !sources.empty())
                result = SourceCheck::Same;
            else if (error.find("HTTP 404") != std::string::npos)
                result = SourceCheck::Missing;
            else if (error == "Unauthorized")
                result = SourceCheck::Unauthorized;
            const DownloadMediaSource* source =
                result == SourceCheck::Same ? &sources.front() : nullptr;
            {
                std::lock_guard<std::mutex> l(m_mutex);
