...
void EpisodeBrowserScreen::fetchEpisodes(bool loadCachedEpisodes)
{
    ...
    m_fetchThread = std::thread([this, s, seriesItem, seasonItem, sid, season, networkOffline,
                                 downloadedOnly, loadCached, libraryQuery, libraryCoordinator,
                                 cancellation, downloads]() {
        ...
        auto downloadedEpisodes = [&](std::vector<MediaItem> items) {
            ...
            for (const auto& download : snapshot.items) {
                if (isComplete(download.state) && download.itemType == "episode" &&
                    !download.itemId.empty() && download.seasonId == season &&
                    (download.seriesId.empty() || download.seriesId == seriesItem.id)) {
        ...
        if (libraryCoordinator) {
            ...
            while (!m_fetchCancelled.load(std::memory_order_acquire) &&
                ...
                if (libraryCoordinator->requestSeasonEpisodes(seriesItem, seasonItem, request)) {
                    accepted = true;
                    break;
                }
...
void EpisodeBrowserScreen::publishEpisodes(std::vector<MediaItem> episodes)
{
    ...
    std::vector<ArtworkJob> jobs;
    jobs.reserve(m_episodes.size());
    for (const auto& ep : m_episodes) {
        ArtworkJob j;
        j.itemId = ep.id;
        auto t = ep.imageTags.find("Primary");
        if (t != ep.imageTags.end() && !t->second.empty()) {
            j.imageTag = t->second;
            j.artworkKey = ep.id + ":Primary:" + t->second + ":288x162";
        }
        jobs.push_back(std::move(j));
    }
    {
        std::lock_guard<std::mutex> g(m_workerMutex);
        m_artworkJobs = std::move(jobs);
        m_failedKeys.clear();
