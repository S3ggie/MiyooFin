# Task 024 — Split JellyfinApi hierarchy endpoints
...
Split JellyfinApi hierarchy endpoints. This task must be behavior-preserving.
...
Create `JellyfinApiHierarchy.cpp`.
Move season/episode/hierarchy endpoint definitions and directly coupled URL-building helpers.
Do not change hierarchy metadata fields or endpoint parameters.

## Behavior That Must Not Change

- Season IndexNumber parsing behavior stays intact.
- Episode metadata fields remain intact.
- Offline hierarchy planners see identical results.
...
```sh
git add src/net/JellyfinApi.cpp src/net/JellyfinApiHierarchy.cpp Makefile
git commit -m "refactor: split jellyfin hierarchy api"
git status --short
```
