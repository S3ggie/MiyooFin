          ...
          const std::vector<std::pair<std::string, std::vector<MediaItem>>>& showsByView)
{
    ...
    for (const auto& pr : moviesByView)
        if (!pr.second.empty())
            movies.rows.push_back({pr.first, pr.second});
    if (movies.rows.empty())
        movies.rows.push_back({"No movies found", {}});
    ...
    for (const auto& pr : showsByView)
        if (!pr.second.empty())
            shows.rows.push_back({pr.first, pr.second});
    if (shows.rows.empty())
        shows.rows.push_back({"No shows found", {}});
    ...
    if (!items.empty()) {
        if (cwIt != rows.end())
            ...
            if (rows.size() == 1 && rows[0].label.empty() && rows[0].items.empty())
                rows.clear();
            auto recentlyAdded = std::find_if(rows.begin(), rows.end(), [](const MediaRow& row) {
                return row.label == "Recently Added";
            });
...
void updateRecentlyAddedRow(std::vector<TabData>& tabs, const std::vector<MediaItem>& items)
{
    auto homeIt = std::find_if(tabs.begin(), tabs.end(),
                               [](const TabData& tab) { return tab.name == "Home"; });
    if (homeIt == tabs.end() || items.empty())
        return;
    auto& rows = homeIt->rows;
    auto recentIt = std::find_if(rows.begin(), rows.end(),
    ...
    if (rows.size() == 1 && rows[0].label.empty() && rows[0].items.empty())
        rows.clear();
    auto continueIt = std::find_if(rows.begin(), rows.end(), [](const MediaRow& row) {
        return row.label == "Continue Watching";
    });
...
HomeMediaWindows mediaWindowsFromTabs(const std::vector<TabData>& tabs)
{
    ...
    for (const auto& tab : tabs) {
        if (tab.rows.empty())
            continue;
        if (tab.name == "Movies")
            out.movies = tab.rows.front().items;
        if (tab.name == "Shows")
            out.shows = tab.rows.front().items;
    }
    return out;
}
...
std::vector<MediaItem> combineMovieViews(const std::vector<CachedLibraryView>& views)
{
    ...
    for (const auto& v : views)
        for (const auto& i : v.items)
            if (seen.emplace(i.id, true).second)
                out.push_back(i);
...
std::vector<TabData> tabsFromSnapshot(const LibrarySnapshot& s)
{
    ...
    if (!s.continueWatching.empty())
        home.push_back({"Continue Watching", s.continueWatching});
    if (!s.recentlyAdded.empty())
        home.push_back({"Recently Added", s.recentlyAdded});
    if (home.empty())
        home.push_back({"", {}});
    ...
    tabs.push_back(std::move(movies));
    TabData shows{"Shows", {}};
    for (const auto& v : s.shows)
        shows.rows.push_back({v.name, v.items});
    if (shows.rows.empty())
        shows.rows.push_back({"No shows found", {}});
    tabs.push_back(std::move(shows));
    tabs.push_back({"Downloads", {{"", {}}}});
...
std::vector<TabData> offlineTabsFromSnapshot(const LibrarySnapshot& snapshot)
{
    ...
    tabs.erase(std::remove_if(
                   tabs.begin(), tabs.end(),
                   [](const TabData& tab) { return tab.name == "Home" || tab.name == "Search"; }),
               tabs.end());
    return tabs;
}
                       ...
                       const std::vector<TabData>& to)
{
    const std::string name =
        (selected >= 0 && selected < (int)from.size()) ? from[selected].name : "";
