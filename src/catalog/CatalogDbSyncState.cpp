...
std::future<CatalogDbSyncState> CatalogDb::readSyncState(bool legacyAvailable,
                                                         std::int64_t legacyLastSuccessfulMs,
                                                         std::int64_t legacyLastReconcileMs,
                                                         const CatalogDbJobMetadata& metadata)
{
    return enqueueSyncStateRead(legacyAvailable, legacyLastSuccessfulMs, legacyLastReconcileMs,
                                metadata);
}
std::future<CatalogDbSyncState> CatalogDb::writeSyncState(std::int64_t lastSuccessfulMs,
                                                          std::int64_t lastReconcileMs,
                                                          std::uint64_t committedGeneration,
                                                          const CatalogDbJobMetadata& metadata)
{
    return enqueueSyncStateWrite(lastSuccessfulMs, lastReconcileMs, committedGeneration, metadata);
}
std::future<CatalogDbSyncState>
CatalogDb::enqueueSyncStateRead(bool legacyAvailable, std::int64_t legacyLastSuccessfulMs,
                                std::int64_t legacyLastReconcileMs,
                                const CatalogDbJobMetadata& metadata)
{
...
std::future<CatalogDbSyncState>
CatalogDb::enqueueSyncStateWrite(std::int64_t lastSuccessfulMs, std::int64_t lastReconcileMs,
                                 std::uint64_t committedGeneration,
                                 const CatalogDbJobMetadata& metadata)
{
    auto command = std::make_shared<SyncStateCommand>();
...
void CatalogDb::processSyncState(const std::shared_ptr<SyncStateCommand>& command)
{
    ...
    if (command->write) {
        ...
        const auto currentGeneration = static_cast<std::uint64_t>(currentGenerationValue);
        sqlite3_finalize(current);
        const bool currentMalformed =
            currentSuccessful < 0 || currentReconcile < 0 || currentGenerationValue < 0 ||
        ...
        const bool prepared = sqlite3_prepare_v2(m_db,
                                                 "UPDATE sync_state SET last_successful_ms=?1, "
                                                 "last_reconcile_ms=?2, committed_generation=?3 "
                                                 "WHERE singleton_id=1",
                                                 -1, &statement, nullptr) == SQLITE_OK;
        const bool updated =
            prepared && sqlite3_bind_int64(statement, 1, command->lastSuccessfulMs) == SQLITE_OK &&
            sqlite3_bind_int64(statement, 2, command->lastReconcileMs) == SQLITE_OK &&
        ...
        if (!updated || !stateIsValid() ||
            sqlite3_exec(m_db, "COMMIT;", nullptr, nullptr, nullptr) != SQLITE_OK) {
    ...
    result.committedGeneration = static_cast<std::uint64_t>(sqlite3_column_int64(statement, 2));
    sqlite3_finalize(statement);

    const bool legacyHasCheckpoint =
        command->legacyLastSuccessfulMs > 0 || command->legacyLastReconcileMs > 0;
    if (command->legacyAvailable && legacyHasCheckpoint && result.lastSuccessfulMs == 0 &&
        result.lastReconcileMs == 0 && result.committedGeneration == 0) {
        if (command->legacyLastSuccessfulMs < 0 || command->legacyLastReconcileMs < 0) {
            result.error = CatalogDbErrorCategory::ConfigurationFailed;
        ...
        const bool seeded =
            prepared && sqlite3_bind_int64(seed, 1, command->legacyLastSuccessfulMs) == SQLITE_OK &&
            sqlite3_bind_int64(seed, 2, command->legacyLastReconcileMs) == SQLITE_OK &&
