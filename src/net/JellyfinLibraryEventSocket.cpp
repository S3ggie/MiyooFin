...
JellyfinLibraryEventRun JellyfinLibraryEvents::waitForRetry(
                                    ...
                                    const std::atomic<bool> &cancelled)
{
    ...
    while (offset < payload.size()
        ...
        std::size_t sent = 0;
        const CURLcode code = curl_easy_send(c, payload.data() + offset,
                                          ...
                                          std::string &error)
{
    ...
    while (bytes.size() >= 2) {
        ...
        if ((first & 0x0f) == 0x9) {
            ...
            if (!sendAll(c, pong, cancelled)) return false;
...
JellyfinLibraryEventRun JellyfinLibraryEvents::run(
    JellyfinLibraryEventQueue &queue,
...
std::future<JellyfinLibraryEventRun> JellyfinLibraryEvents::start(
