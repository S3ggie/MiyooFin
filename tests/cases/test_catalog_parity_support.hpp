...
bool catalogParityItemEqual(const MediaItem& expected, const MediaItem& actual)
{
    const bool equal =
        expected.id == actual.id && expected.title == actual.title &&
        expected.overview == actual.overview && expected.year == actual.year &&
        expected.rating == actual.rating && expected.genre == actual.genre &&
        expected.type == actual.type && expected.etag == actual.etag &&
        expected.genres == actual.genres && expected.played == actual.played &&
        ...
        expected.runTimeTicks == actual.runTimeTicks && expected.seriesName == actual.seriesName &&
        expected.seriesId == actual.seriesId && expected.seasonId == actual.seasonId;
    if (!equal) {
                            ...
                            const std::string& title)
{
    ...
    item.imageTags = {{"Backdrop", "backdrop-" + id}, {"Primary", "primary-" + id}};
    item.indexNumber = 1;
...
bool configureCatalogParityDb(CatalogDb& db, const std::string& url, const std::string& user)
{
...
static std::vector<std::string> collectCatalogPageIds(CatalogDb& db, const std::string& type,
                                                      int letter, std::size_t pageSize,
                                                      std::uint64_t epoch)
{
    std::vector<std::string> ids;
    CatalogDbPageCursor cursor;
    for (std::size_t page = 0; page < 4096; ++page) {
        const auto result = db.readMediaPage(type, letter, pageSize, cursor, {0, epoch, {}}).get();
        CHECK(result.success && result.workerOwned && !result.cancelled && !result.superseded);
