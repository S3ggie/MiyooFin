# Task 21 — Validate deleted library view

**Phase:** C — validation

## Objective

Prove a supported view absent from a later complete generation is removed from committed descriptors and cannot keep former items browse-authoritative.

## Why

Authoritative finalization must sweep stale views only after success.
...
1. Commit generation A with two supported views.
2. Commit generation B with one surviving view.
3. Read view descriptors and media pages.

## Tests

- Deleted view gone.
- Items only in deleted view are not returned.
- Items also in surviving view remain once.
...
```text
test(catalog): cover deleted library view
```

Do not include any part of Task 22 in this commit.
