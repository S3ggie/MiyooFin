# Task 23 — Validate multiple TV libraries

**Phase:** C — validation

## Objective

Prove multiple supported `tvshows` views coexist and page/deduplicate correctly.

## Why

Multiple TV libraries were a required audit scenario and exercise later-view truncation/membership handling.
...
1. Commit two TV views with distinct items and one shared item.
2. Read show pages and returned memberships.

## Tests

- All unique series reachable.
- Shared series once with both memberships.
- Later TV view not truncated.
...
```text
test(shows): cover multiple tv libraries
```

Do not include any part of Task 24 in this commit.
