...
EpisodeBrowserScreen::EpisodeBrowserScreen(
    const Session& session, const MediaItem& series, const MediaItem& season,
    ...
    : m_session(session), m_series(series), m_season(season), m_initialEpisodeId(initialEpisodeId),
      m_downloads(std::move(downloads)), m_libraryCoordinator(std::move(libraryCoordinator)),
      m_libraryQuery(std::move(libraryQuery)), m_networkOffline(networkOffline),
...
// -------------------------------------------------------------------
int EpisodeBrowserScreen::findEpisodeIndex(const std::vector<MediaItem>& episodes,
...
int EpisodeBrowserScreen::nextPrefetchIndex(int selected, int listScroll, int total,
                                            const std::set<int>& unavailable)
{
    if (selected < 0 || selected >= total)
        return -1;
    if (!unavailable.count(selected))
        return selected;
    ...
    for (int distance = 1; distance < LIST_VISIBLE; ++distance) {
        ...
        if (ahead < last && !unavailable.count(ahead))
            return ahead;
        const int behind = selected - distance;
        if (behind >= first && !unavailable.count(behind))
            return behind;
    }
    return -1;
}

bool EpisodeBrowserScreen::artworkRequestCancelled(bool cancellationRequested,
...
bool EpisodeBrowserScreen::shouldMarkArtworkFailed(bool success, bool cancelled)
{
    return !success && !cancelled;
}

bool EpisodeBrowserScreen::advancePrefetchResume(bool& pending, int& delayUpdates)
{
    if (!pending)
        return false;
    if (delayUpdates > 0) {
...
void EpisodeBrowserScreen::clampListScroll()
{
    ...
    if (maxScroll < 0)
        maxScroll = 0;
    if (m_listScroll > maxScroll)
        m_listScroll = maxScroll;
}
...
void EpisodeBrowserScreen::update(Uint32 /*dt*/)
{
    ...
    bool cachedDone = false;
    {
        ...
        if (m_cachedEpisodesDone) {
            cached = std::move(m_cachedEpisodes);
    ...
    if (fetchDone) {
        UiDiagnostics::Scope scope("EpisodeBrowserScreen::publishFetchResult");
        ...
        bool ok;
        {
            std::lock_guard<std::mutex> g(m_fetchMutex);
            ok = m_fetchOk;
            fresh = std::move(m_fetchEpisodes);
            err = m_fetchError;
            m_fetchDone = false;
        }
        if (m_fetchThread.joinable())
            m_fetchThread.join();
        if (ok)
            publishEpisodes(std::move(fresh));
        else if (m_episodes.empty()) {
