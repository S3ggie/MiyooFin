...
void MovieDetailsScreen::renderContent(SDL_Surface* fb)
{
    ...
    BitmapFont::fillRect(fb, POSTER_X, POSTER_Y, POSTER_W, POSTER_H, color.r, color.g, color.b,
                         color.a);

    // 2. Draw decoded artwork (aspect-fit, centered, no crop/stretch)
    SDL_Surface* artworkSurface =
        m_movieArtworkSurface ? m_movieArtworkSurface : m_gridArtworkSurface;
    ...
    BitmapFont::drawRect(fb, POSTER_X, POSTER_Y, POSTER_W, POSTER_H, Theme::TEXT_R, Theme::TEXT_G,
                         Theme::TEXT_B);
    ...
    // Movie title
    BitmapFont::drawString(fb, rx, ry, m_movie.title.c_str(), Theme::ACCENT_R, Theme::ACCENT_G,
    ...
    if (m_movie.rating > 0.0f) {
        int len = (int)std::strlen(metaBuf);
        if (hasMeta)
            std::snprintf(metaBuf + len, sizeof(metaBuf) - len, "  |  %.1f/10",
                          (double)m_movie.rating);
        else {
            std::snprintf(metaBuf, sizeof(metaBuf), "%.1f/10", (double)m_movie.rating);
            hasMeta = true;
        }
    }
    if (hasMeta) {
        BitmapFont::drawString(fb, rx, ry, metaBuf, Theme::TEXT_R, Theme::TEXT_G, Theme::TEXT_B,
    ...
    if (!m_movie.genres.empty()) {
        ...
        BitmapFont::drawString(fb, rx, ry, genreStr.c_str(), Theme::TEXT_R, Theme::TEXT_G,
                               Theme::TEXT_B, Theme::BG_R, Theme::BG_G, Theme::BG_B);
        ry += BitmapFont::GLYPH_H + 4;
    } else if (!m_movie.genre.empty()) {
        BitmapFont::drawString(fb, rx, ry, m_movie.genre.c_str(), Theme::TEXT_R, Theme::TEXT_G,
                               Theme::TEXT_B, Theme::BG_R, Theme::BG_G, Theme::BG_B);
        ry += BitmapFont::GLYPH_H + 4;
    }
    ...
    if (!m_movie.overview.empty()) {
        ...
        for (int i = m_overviewScroll;
             i < m_overviewScroll + visibleLines && i < (int)m_overviewLines.size(); ++i) {
            BitmapFont::drawString(fb, rx, drawY, m_overviewLines[i].c_str(), Theme::TEXT_R,
    ...
    if (m_downloads && m_planId) {
        UiDiagnostics::Scope stateScope(
            "MovieDetailsScreen::render playback/download state preparation");
        ...
        if (m_confirmDownload)
            ...
            status =
                p.plan.sizeKnown
                    ? "Download: ~" + formatBytes(p.plan.additionalRequiredBytes) + "  Preparing..."
                    : "Checking size...";
            ...
            status = p.plan.error;
        if (!status.empty())
            BitmapFont::drawString(fb, rx, BTN_Y - 20, status.c_str(), Theme::ACCENT_R,
    ...
    // 7. Bottom hint bar
    renderBottomHints(fb, overviewScrollable ? "A=Select  B=Back  L/R=Bio" : "A=Select  B=Back");
}
}
