...
bool parseSemVer(const std::string &in, SemVer &out)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (val > static_cast<unsigned long>(INT32_MAX)) return false;
        // Reject leading zeros (semver: numeric identifiers must not have leading zeros, except for "0" itself)
        if (pos - digitStart > 1 && s[digitStart] == '0') return false;
    ...
    if (hadDash && pre.empty()) return false; // trailing dash with no prerelease
    if (!pre.empty()) {
        for (char c : pre) {
        ...
        if (prevDot) return false; // ends with dot
        ...
        while (i < pre.size()) {
            size_t start = i;
            while (i < pre.size() && pre[i] != '.') ++i;
            size_t len = i - start;
            if (len > 1 && pre[start] == '0') {
                bool purelyNumeric = std::all_of(
                    pre.begin() + start, pre.begin() + start + len,
                    [](char c){ return std::isdigit(
                        static_cast<unsigned char>(c)); });
                if (purelyNumeric) return false;
            }
            ++i; // skip dot
        }
    }
...
{
    if (a == b) return 0;
    if (a.empty() && !b.empty()) return 1;  // release > prerelease
    ...
    while (pa < a.size() || pb < b.size()) {
        ...
        std::string idA, idB;
        while (pa < a.size() && a[pa] != '.') idA += a[pa++];
        while (pb < b.size() && b[pb] != '.') idB += b[pb++];
        if (pa < a.size()) pa++; // skip dot
        if (pb < b.size()) pb++; // skip dot
...
int compareSemVer(const SemVer &a, const SemVer &b)
{
    ...
    if (a.prerelease.empty()) return 1;  // a is release, b is prerelease
    if (b.prerelease.empty()) return -1; // a is prerelease, b is release
...
{
    SemVer cv, cv2;
    if (!parseSemVer(candidate, cv)) return false;
    if (!parseSemVer(current, cv2)) return false;
