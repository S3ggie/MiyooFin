...
CatalogDb::reconcileSeries(const std::vector<MediaItem>& series, bool authoritative,
                           const CatalogDbJobMetadata& metadata)
{
    return enqueueReconcile(series, authoritative, metadata);
}
std::future<CatalogDbReconcileResult>
CatalogDb::enqueueReconcile(const std::vector<MediaItem>& series, bool authoritative,
...
void CatalogDb::processReconcile(const std::shared_ptr<ReconcileCommand>& command)
{
    ...
    if (!prepareCached("hierarchy_upsert_item",
                       ...
                       upsert) ||
        !prepareCached("reconcile_clear_ids", "DELETE FROM catalog_reconcile_ids", clearIds) ||
        !prepareCached("reconcile_stage_id", "INSERT INTO catalog_reconcile_ids(id) VALUES(?1)",
                       stageId) ||
        !prepareCached("reconcile_delete_absent",
    ...
    auto writeSeries = [&](const MediaItem& item) {
        ...
        if (command->failureSpec().failAfterRows >= 0 &&
            result.seriesUpserted >=
                static_cast<std::size_t>(command->failureSpec().failAfterRows)) {
            result.error = CatalogDbErrorCategory::SqliteError;
        ...
        reset(stageId);
        const bool staged =
            sqlite3_bind_text(stageId, 1, item.id.c_str(), -1, SQLITE_TRANSIENT) == SQLITE_OK &&
    ...
    if (!began || !executeNoParameter(clearIds)) {
        rollback();
        finish();
        return;
    }
    ...
    performanceTelemetry().addCatalogDbRows(0, 0, static_cast<uint32_t>(result.seriesDeleted));
    reset(deleteAbsent);
...
void CatalogDb::processTopLevelSync(const std::shared_ptr<TopLevelSyncCommand>& command)
{
    assert(std::this_thread::get_id() == m_worker.get_id());
    ...
    if (!m_scopeConfigured || !m_scopeReady || command->metadata.scopeEpoch == 0 ||
        command->metadata.scopeEpoch != m_requestedEpoch || command->generation == 0) {
        result.error = CatalogDbErrorCategory::ScopeNotReady;
    ...
    const char* const createSql[] = {
        "CREATE TEMP TABLE IF NOT EXISTS top_level_sync_views (generation INTEGER NOT NULL, id "
        "TEXT NOT NULL, name TEXT NOT NULL, collection_type TEXT NOT NULL, ordinal INTEGER NOT "
        "NULL, PRIMARY KEY(generation, id))",
        "CREATE TEMP TABLE IF NOT EXISTS top_level_sync_membership (generation INTEGER NOT NULL, "
        "view_id TEXT NOT NULL, item_id TEXT NOT NULL, ordinal INTEGER NOT NULL, PRIMARY "
        "KEY(generation, view_id, item_id))",
        "CREATE TEMP TABLE IF NOT EXISTS top_level_sync_active (generation INTEGER NOT NULL "
        "PRIMARY KEY)",
    };
    ...
    if (command->begin) {
        if (!exec(m_db,
                  "DELETE FROM top_level_sync_views; DELETE FROM top_level_sync_membership; DELETE "
                  "FROM top_level_sync_active;",
                  error)) {
            result.error = CatalogDbErrorCategory::SqliteError;
            result.message = error;
            command->result.set_value(std::move(result));
        ...
        if (sqlite3_prepare_v2(m_db, "INSERT INTO top_level_sync_active(generation) VALUES(?1)", -1,
                               &statement, nullptr) != SQLITE_OK ||
            sqlite3_bind_int64(statement, 1, static_cast<sqlite3_int64>(command->generation)) !=
                SQLITE_OK ||
        ...
        const std::string copyViews =
            "INSERT INTO library_views(id,name,collection_type,ordinal) SELECT "
            "id,name,collection_type,ordinal FROM top_level_sync_views WHERE generation=" +
            std::to_string(command->generation) + " ORDER BY ordinal,id;";
        const std::string copyMembership =
            "INSERT INTO library_membership(view_id,item_id,ordinal) SELECT "
            "view_id,item_id,ordinal FROM top_level_sync_membership WHERE generation=" +
        ...
        if (!exec(m_db, "BEGIN IMMEDIATE;", error) ||
            ...
            !exec(m_db, copyViews.c_str(), error) || !exec(m_db, copyMembership.c_str(), error) ||
        ...
        exec(m_db,
             "DELETE FROM top_level_sync_views; DELETE FROM top_level_sync_membership; DELETE FROM "
             "top_level_sync_active;",
             error);
...
void CatalogDb::processLibrarySeed(const std::shared_ptr<LibrarySeedCommand>& command)
{
    CatalogCompatibilitySeedResult result;
    result.workerOwned = true;
    auto stale = [&] {
        std::lock_guard<std::mutex> lock(m_mutex);
        return command->metadata.generation != m_generation ||
               (command->metadata.scopeEpoch != 0 &&
                (command->metadata.scopeEpoch != m_requestedEpoch || !m_scopeConfigured ||
                 !m_scopeReady));
    };
    if (command->metadata.cancellation && command->metadata.cancellation->load()) {
        result.cancelled = true;
        result.error = CatalogDbErrorCategory::Superseded;
        result.message = "library seed cancelled";
        command->result.set_value(std::move(result));
        return;
    }
    if (stale()) {
        result.superseded = true;
        result.error = CatalogDbErrorCategory::Superseded;
        result.message = "library seed superseded";
        command->result.set_value(std::move(result));
        return;
    }
        ...
        for (const auto& view : views)
            for (const auto& item : view.items) {
                ...
                if (found != canonical.end()) {
                    if (!mediaItemsEquivalentForCatalog(found->second, item))
                        return false;
                } else {
                    canonical.emplace(item.id, item);
                    items.push_back(item);
                }
            }
        return true;
    };
    if (!collect(command->request.snapshot.movies) || !collect(command->request.snapshot.shows)) {
        result.error = CatalogDbErrorCategory::CorruptOrIo;
        result.message = "invalid library snapshot";
        command->result.set_value(std::move(result));
        return;
    }
    auto collectHome = [&](const std::vector<MediaItem>& row) {
    ...
    if (!collectHome(command->request.snapshot.continueWatching) ||
        !collectHome(command->request.snapshot.recentlyAdded)) {
        result.error = CatalogDbErrorCategory::CorruptOrIo;
        result.message = "invalid Home snapshot";
        command->result.set_value(std::move(result));
        return;
    }
    auto execSeed = [&](const std::string& sql) {
        char* err = nullptr;
        int rc = sqlite3_exec(m_db, sql.c_str(), nullptr, nullptr, &err);
        if (rc != SQLITE_OK) {
            result.message = err ? err : sqlite3_errmsg(m_db);
            sqlite3_free(err);
            return false;
        }
        return true;
    };
    if (!execSeed("BEGIN IMMEDIATE;")) {
        result.error = CatalogDbErrorCategory::SqliteError;
        command->result.set_value(std::move(result));
        return;
    }
    auto rollback = [&] {
        char* e = nullptr;
        sqlite3_exec(m_db, "ROLLBACK;", nullptr, nullptr, &e);
        sqlite3_free(e);
    };
    sqlite3_stmt* upsert = nullptr;
    sqlite3_stmt *deleteGenres = nullptr, *insertGenre = nullptr, *deleteTags = nullptr,
                 *insertTag = nullptr;
    const char* sql =
        "INSERT INTO "
    ...
    if (sqlite3_prepare_v2(m_db, sql, -1, &upsert, nullptr) != SQLITE_OK ||
        ...
        sqlite3_prepare_v2(m_db, "DELETE FROM item_image_tags WHERE item_id=?", -1, &deleteTags,
                           nullptr) != SQLITE_OK ||
        sqlite3_prepare_v2(m_db,
                           "INSERT INTO item_image_tags(item_id,image_type,tag) VALUES(?,?,?)", -1,
                           &insertTag, nullptr) != SQLITE_OK) {
        result.error = CatalogDbErrorCategory::SqliteError;
        result.message = sqlite3_errmsg(m_db);
        rollback();
        command->result.set_value(std::move(result));
    ...
    const MediaItemCollectionStatements collections{deleteGenres, insertGenre, deleteTags,
                                                    insertTag};
    int writes = 0;
    for (const auto& item : items) {
        ...
        if (!bindMediaItemScalars(upsert, item, e) || sqlite3_step(upsert) != SQLITE_DONE ||
            !maintainOrganizationalSortKey(m_db, item, result.message) ||
            !replaceMediaItemCollections(collections, item, e)) {
            result.error = CatalogDbErrorCategory::SqliteError;
            result.message = sqlite3_errmsg(m_db);
        ...
        if (command->failureSpec().failAfterWrites >= 0 &&
            ++writes >= command->failureSpec().failAfterWrites) {
            result.error = CatalogDbErrorCategory::SqliteError;
            result.message = "injected library seed failure";
            sqlite3_finalize(upsert);
            sqlite3_finalize(deleteGenres);
            sqlite3_finalize(insertGenre);
            sqlite3_finalize(deleteTags);
    ...
    if (!execSeed(
            "DELETE FROM library_membership; DELETE FROM library_views; DELETE FROM home_items;")) {
        result.error = CatalogDbErrorCategory::SqliteError;
        rollback();
        command->result.set_value(std::move(result));
        return;
    }
    auto addView = [&](const CachedLibraryView& v, int ordinal) {
        sqlite3_stmt* s = nullptr;
        if (sqlite3_prepare_v2(
                m_db, "INSERT INTO library_views(id,name,collection_type,ordinal) VALUES(?,?,?,?)",
                -1, &s, nullptr) != SQLITE_OK)
            return false;
        sqlite3_bind_text(s, 1, v.id.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(s, 2, v.name.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(s, 3, v.collectionType.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(s, 4, ordinal);
        bool ok = sqlite3_step(s) == SQLITE_DONE;
        sqlite3_finalize(s);
        if (!ok)
            return false;
        for (size_t i = 0; i < v.items.size(); ++i) {
            if (!execSeed("INSERT INTO library_membership(view_id,item_id,ordinal) VALUES('" +
                          v.id + "','" + v.items[i].id + "'," + std::to_string(i) + ")"))
                return false;
        }
        ++result.viewsWritten;
        return true;
    };
    for (size_t i = 0; i < command->request.snapshot.movies.size(); ++i)
        if (!addView(command->request.snapshot.movies[i], i)) {
            result.error = CatalogDbErrorCategory::SqliteError;
            rollback();
            command->result.set_value(std::move(result));
            return;
        }
    for (size_t i = 0; i < command->request.snapshot.shows.size(); ++i)
        if (!addView(command->request.snapshot.shows[i],
                     i + command->request.snapshot.movies.size())) {
            result.error = CatalogDbErrorCategory::SqliteError;
            rollback();
            command->result.set_value(std::move(result));
            return;
        }
    auto addHome = [&](const char* kind, const std::vector<MediaItem>& v) {
        for (size_t i = 0; i < v.size(); ++i)
            if (!execSeed("INSERT INTO home_items(row_kind,item_id,ordinal) VALUES('" +
                          std::string(kind) + "','" + v[i].id + "'," + std::to_string(i) + ")"))
                return false;
        result.homeItemsWritten += v.size();
        return true;
    };
    if (!addHome("continue_watching", command->request.snapshot.continueWatching) ||
        !addHome("recently_added", command->request.snapshot.recentlyAdded) || stale() ||
        !execSeed("COMMIT;")) {
        result.error = CatalogDbErrorCategory::Superseded;
        rollback();
        command->result.set_value(std::move(result));
