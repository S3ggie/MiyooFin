...
bool reconcileSource(DownloadItem& item, SourceCheck result,
                     const DownloadMediaSource* source = nullptr);
...
inline bool applyReconciledSource(DownloadItem& live, SourceCheck result,
                                  const DownloadMediaSource* source, std::uint64_t nowMs)
{
...
// checked.
inline bool reconcileShouldSkip(const DownloadItem& item, bool startup, std::uint64_t nowMs)
{
    if (item.state != DownloadState::Complete)
        return false;
    if (startup)
        return true;
    return item.lastVerifiedMs > 0 && (nowMs - item.lastVerifiedMs) < RECONCILE_VERIFY_TTL_MS;
}
}
#endif
