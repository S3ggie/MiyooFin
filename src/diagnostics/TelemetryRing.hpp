...
template <std::size_t Capacity> class TelemetryRing
{
  ...
  public:
    ...
    bool tryEnqueue(const TelemetryRecord& record) noexcept
    {
        ...
        for (unsigned int reservationAttempt = 0; reservationAttempt < 8; ++reservationAttempt) {
            ...
            slot.record = record;
            const uint32_t depth = m_approximateDepth.fetch_add(1, std::memory_order_relaxed) + 1;
    ...
    bool tryDequeue(TelemetryRecord& record) noexcept
    {
        ...
        record = slot.record;
        slot.sequence.store(position + static_cast<uint32_t>(Capacity), std::memory_order_release);
    ...
    void seedPositionsForTest(uint32_t position) noexcept
    {
        ...
        for (std::size_t index = 0; index < Capacity; ++index) {
            const uint32_t offset =
                (static_cast<uint32_t>(index) + static_cast<uint32_t>(Capacity) - firstSlot) &
                static_cast<uint32_t>(Capacity - 1);
            m_slots[index].sequence.store(position + offset, std::memory_order_relaxed);
        }
  ...
  private:
    void updateHighwater(uint32_t depth) noexcept
    {
        ...
        for (unsigned int attempt = 0; attempt < 8 && depth > observed; ++attempt) {
            if (m_approximateHighwater.compare_exchange_weak(
