...
inline bool isAnimeSeries(const std::string &viewName,const MediaItem &item) { if(libraryNameContainsAnimeToken(viewName))return true; for(const auto &g:item.genres)if(asciiEqualsAnime(g))return true; return false; }
inline bool isAnimeSeries(const CachedLibraryView &view,const MediaItem &item) { return isAnimeSeries(view.name,item); }
