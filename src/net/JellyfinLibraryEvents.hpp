...
class JellyfinLibraryEvents
{
  public:
    ...
    static JellyfinLibraryEventParse parseMessage(const std::string& message,
                                                  JellyfinLibraryChangeBatch& batch);

    static std::string buildSocketUrl(const std::string& baseUrl, const std::string& accessToken,
    ...
    class Backoff
    {
      public:
        ...
        unsigned nextDelayMs()
        {
            ...
            m_current = std::min(m_maximum, m_current > m_maximum / 2 ? m_maximum : m_current * 2);
            return delay;
        }
        void reset()
        {
    ...
    static JellyfinLibraryEventRun waitForRetry(const std::atomic<bool>& cancelled,
                                                unsigned delayMs);
    ...
    JellyfinLibraryEventRun run(JellyfinLibraryEventQueue& queue,
