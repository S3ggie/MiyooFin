...
int bindOptionalText(sqlite3_stmt* statement, int parameter, const std::string& value)
{
    if (value.empty()) {
        return sqlite3_bind_null(statement, parameter);
    }
    return sqlite3_bind_text(statement, parameter, value.c_str(), -1, SQLITE_TRANSIENT);
}
...
bool bindItemId(sqlite3_stmt* statement, const MediaItem& item, MediaItemSqlError& error)
{
    if (item.id.empty()) {
        error = MediaItemSqlError::MissingId;
        return false;
    }
    return bindResult(sqlite3_bind_text(statement, 1, item.id.c_str(), -1, SQLITE_TRANSIENT),
                      error);
}
...
bool bindMediaItemScalars(sqlite3_stmt* statement, const MediaItem& item, MediaItemSqlError& error)
{
    ...
    return bindResult(sqlite3_bind_text(statement, 1, item.id.c_str(), -1, SQLITE_TRANSIENT),
           ...
           bindResult(sqlite3_bind_text(statement, 3, item.title.c_str(), -1, SQLITE_TRANSIENT),
                      error) &&
           bindResult(sqlite3_bind_text(statement, 4, item.overview.c_str(), -1, SQLITE_TRANSIENT),
                      error) &&
           bindResult(sqlite3_bind_int(statement, 5, item.year), error) &&
           bindResult(sqlite3_bind_double(statement, 6, item.rating), error) &&
           bindResult(sqlite3_bind_text(statement, 7, item.etag.c_str(), -1, SQLITE_TRANSIENT),
                      error) &&
           bindResult(sqlite3_bind_int(statement, 8, item.played ? 1 : 0), error) &&
           bindResult(sqlite3_bind_double(statement, 9, item.progress), error) &&
           bindResult(sqlite3_bind_int64(statement, 10, item.playbackPositionTicks), error) &&
           bindResult(sqlite3_bind_int(statement, 11, item.indexNumber), error) &&
           bindResult(sqlite3_bind_int(statement, 12, item.parentIndexNumber), error) &&
           bindResult(sqlite3_bind_int64(statement, 13, item.runTimeTicks), error) &&
           bindResult(
               sqlite3_bind_text(statement, 14, item.seriesName.c_str(), -1, SQLITE_TRANSIENT),
               error) &&
...
bool readMediaItemScalars(sqlite3_stmt* statement, MediaItem& item, MediaItemSqlError& error)
{
    ...
    item.type = mediaItemKindFromSql(sqlite3_column_int(statement, 1), kindError);
                                 ...
                                 const MediaItem& item, MediaItemSqlError& error)
{
    ...
    for (std::size_t ordinal = 0; ordinal < item.genres.size(); ++ordinal) {
        ...
        if (!bindResult(
                sqlite3_bind_text(statements.insertGenre, 1, item.id.c_str(), -1, SQLITE_TRANSIENT),
                error) ||
            !bindResult(sqlite3_bind_int(statements.insertGenre, 2, static_cast<int>(ordinal)),
                        error) ||
            !bindResult(sqlite3_bind_text(statements.insertGenre, 3, item.genres[ordinal].c_str(),
                                          -1, SQLITE_TRANSIENT),
                        error) ||
    ...
    if (!item.imageTags.empty()) {
        ...
        for (const auto& tag : item.imageTags) {
            ...
            if (!bindResult(sqlite3_bind_text(statements.insertImageTag, 1, item.id.c_str(), -1,
                                              SQLITE_TRANSIENT),
                            error) ||
                !bindResult(sqlite3_bind_text(statements.insertImageTag, 2, tag.first.c_str(), -1,
                                              SQLITE_TRANSIENT),
                            error) ||
                !bindResult(sqlite3_bind_text(statements.insertImageTag, 3, tag.second.c_str(), -1,
                                              SQLITE_TRANSIENT),
                            error) ||
...
bool mediaItemsEquivalentForCatalog(const MediaItem& expected, const MediaItem& actual)
{
    return expected.id == actual.id && expected.title == actual.title &&
           ...
           expected.imageTags == actual.imageTags && expected.indexNumber == actual.indexNumber &&
