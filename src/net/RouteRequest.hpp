...
#if defined(MIYOOFIN_TELEMETRY_HOST_TEST) && !defined(MIYOOFIN_TELEMETRY_RING_TEST)
...
class RouteRequest
{
  public:
    ...
    std::string primary() const
    {
        ...
        return local.empty() ? publicRoute() : local;
    }
    bool usesLan() const
    {
        return !lan().empty();
    }
    static bool transportFailure(const std::string& error)
    {
        return error.compare(0, 11, "Transport: ") == 0 &&
               error.find("Callback aborted") == std::string::npos;
    }

    template <typename Operation> bool run(Operation operation, std::string& error) const
    {
        const std::string local = lan();
        ...
        if (!transportFailure(error) || publicUrl.empty() || publicUrl == local)
            return false;
    ...
    static std::string replaceBase(const std::string& url, const std::string& from,
                                   const std::string& to)
    {
