...
{
    std::string itemId, itemType, sourceMode;
    std::int64_t positionTicks = 0, baseResumeTicks = 0;
    bool serverReported = true;
};
...
class PlaybackRequest
{
  public:
    ...
    static bool write(const std::string& itemId, const std::string& itemType, long long resumeTicks,
                      std::string& error);
    ...
    static bool writeTo(const std::string& path, const std::string& itemId,
                        const std::string& itemType, long long resumeTicks, std::string& error);
    ...
    static bool consumeResult(const std::string& expectedItemId, std::int64_t& positionTicks,
                              std::string& error);

    /// Testable explicit-path form of consumeResult().
    static bool consumeResultFrom(const std::string& path, const std::string& expectedItemId,
