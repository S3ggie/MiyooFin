...
enum class DownloadHierarchyRowKind
{
    Movie,
    Series,
    Season,
...
inline std::string downloadHierarchySortTitle(std::string value)
{
    ...
    for (const char* article : {"the ", "an ", "a "})
        if (lower.rfind(article, 0) == 0)
            return lower.substr(std::char_traits<char>::length(article));
    return lower;
}
...
{
    DownloadAggregate out;
    out.episodes = items.size();
    out.bytesKnown = !items.empty();
    unsigned long long progressSum = 0;
    for (const DownloadItem* p : items) {
        const DownloadItem& i = *p;
        if (downloadHierarchyComplete(i))
            ++out.complete;
        if (i.state == DownloadState::Downloading)
            ++out.active;
        ...
        if (!predicted)
            out.bytesKnown = false;
        else
            out.totalBytes = saturatingAdd(out.totalBytes, predicted);
        progressSum += downloadPercent(i); // child progress is the worker's real progress
    }
    out.progressKnown = !items.empty();
    if (out.progressKnown)
        out.progress = (unsigned)(progressSum / items.size());
    return out;
}
...
{
    return !i.seasonId.empty() ? i.seasonId
                               : "number:" + std::to_string(i.seasonNumber) + ":" + i.seasonName;
}

inline bool downloadHierarchyIsBulkRemovalParent(const DownloadHierarchyRow& row)
{
    return row.kind == DownloadHierarchyRowKind::Series ||
...
downloadHierarchyBulkRemovalItemIds(const DownloadHierarchyRow& row,
                                    const DownloadSnapshot& snapshot)
{
    std::vector<std::string> ids;
    if (!downloadHierarchyIsBulkRemovalParent(row) || row.id.rfind("series:", 0) != 0)
        return ids;
    const size_t seasonAt = row.id.find("/season:");
    const std::string seriesRowId =
        seasonAt == std::string::npos ? row.id : row.id.substr(0, seasonAt);
    ...
    for (const DownloadItem& item : snapshot.items) {
        if (item.itemType != "episode" || item.itemId.empty())
            continue;
        if ("series:" + downloadHierarchySeriesKey(item) != seriesRowId)
            continue;
        if (!seasonKey.empty() && downloadHierarchySeasonKey(item) != seasonKey)
            continue;
                                                ...
                                                const std::set<std::string>& expanded)
{
    ...
    {
        std::string key, name;
        std::map<std::string, Season> seasons;
    };
    std::map<std::string, Series> series;
    for (const DownloadItem& i : snapshot.items) {
        if (i.itemType != "episode") {
            out.movies.push_back(
                {DownloadHierarchyRowKind::Movie, "movie:" + i.itemId, i.title, 0, &i, {}});
            continue;
        }
        ...
        s.key = sk;
        if (s.name.empty())
            s.name = i.seriesName.empty() ? "Unknown Series" : i.seriesName;
        Season& season = s.seasons[sek];
        season.key = sek;
        season.number = i.seasonNumber;
        if (season.name.empty())
            season.name = i.seasonName.empty()
                              ? (i.seasonNumber > 0 ? "Season " + std::to_string(i.seasonNumber)
                                                    : "Unknown Season")
                              : i.seasonName;
        season.episodes.push_back(&i);
    }
    std::sort(out.movies.begin(), out.movies.end(), [](const auto& a, const auto& b) {
        return a.item->createdAt == b.item->createdAt
                   ? downloadHierarchySortTitle(a.title) < downloadHierarchySortTitle(b.title)
                   : a.item->createdAt > b.item->createdAt;
    });
    std::vector<Series*> ordered;
    for (auto& entry : series)
        ordered.push_back(&entry.second);
    std::sort(ordered.begin(), ordered.end(), [](const Series* a, const Series* b) {
        return downloadHierarchySortTitle(a->name) < downloadHierarchySortTitle(b->name);
    });
    for (Series* s : ordered) {
        std::vector<const DownloadItem*> all;
        for (auto& e : s->seasons)
            all.insert(all.end(), e.second.episodes.begin(), e.second.episodes.end());
        const std::string sid = "series:" + s->key;
        DownloadHierarchyRow parent{DownloadHierarchyRowKind::Series,
                                    sid,
                                    s->name,
                                    0,
                                    nullptr,
                                    downloadHierarchyAggregate(all),
                                    expanded.count(sid) != 0};
        out.shows.push_back(parent);
        std::vector<Season*> seasons;
        for (auto& e : s->seasons)
            seasons.push_back(&e.second);
        ...
        for (Season* season : seasons) {
            std::sort(season->episodes.begin(), season->episodes.end(),
                      [](const DownloadItem* a, const DownloadItem* b) {
                          ...
                          return downloadHierarchySortTitle(a->title) <
                                 downloadHierarchySortTitle(b->title);
                      });
            const std::string seasonId = sid + "/season:" + season->key;
            DownloadHierarchyRow child{DownloadHierarchyRowKind::Season,
                                       seasonId,
                                       season->name,
                                       1,
                                       nullptr,
                                       downloadHierarchyAggregate(season->episodes),
                                       expanded.count(seasonId) != 0};
            if (parent.expanded)
                out.shows.push_back(child);
            if (parent.expanded && child.expanded)
                for (const DownloadItem* episode : season->episodes)
                    out.shows.push_back({DownloadHierarchyRowKind::Episode,
    ...
    out.visible = out.movies;
    out.visible.insert(out.visible.end(), out.shows.begin(), out.shows.end());
    return out;
}
