...
struct LibrarySyncSchedule
{
    ...
    bool request(Uint32 now)
    {
        ...
        if (hasSucceeded && now - lastSuccess < FRESH_MS)
            return false;
        if (hasAttempted && now - lastAttempt < RETRY_DELAY_MS)
            return false;
        inFlight = true;
        hasAttempted = true;
        lastAttempt = now;
...
inline bool homeChangeNeedsPublication(bool catchUpRequired, bool catalogItemAffectsHome,
...
/// Pure helper — testable without a HomeScreen instance.
inline bool homeRailRefreshDebounced(std::int64_t nowMs, std::int64_t lastSuccessMs,
...
inline std::string librarySyncStatus(int tab, bool haveCache, bool offline, bool metadataActive,
                                     bool syncSucceeded, const ShowsSyncProgress& shows = {},
                                     bool hierarchyActive = false, bool showsTab = false)
{
    (void)showsTab;
    if (tab < 0 || tab > 2)
        return "";
...
inline HomeStartupSync decideHomeStartupSync(std::int64_t nowMs, std::int64_t lastSuccessfulMs,
                                             ...
                                             bool scopeEpochValid, bool catalogHasRows)
{
    ...
    if (committedGeneration == 0)
        return HomeStartupSync::FullReconcile;
    if (lastReconcileMs > lastSuccessfulMs)
        return HomeStartupSync::FullReconcile;
    if (nowMs < lastSuccessfulMs)
        return HomeStartupSync::FullReconcile;
