    ...
    while (pos < headers.size()) {
        ...
        if (line_len >= name_len) {
            ...
            if (match && headers[pos + name_len] == ':') {
                ...
                while (val_start < eol && headers[val_start] == ' ')
                    ++val_start;
                size_t val_end = eol;
                if (val_end > val_start && headers[val_end - 1] == '\r')
                    --val_end;
...
inline bool is_allowed_response_header(const char* name, size_t name_len)
{
    ...
    return ci_eq(name, name_len, "Content-Type") || ci_eq(name, name_len, "Content-Length") ||
