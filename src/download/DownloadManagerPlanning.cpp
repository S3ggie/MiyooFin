...
DownloadPlan DownloadManager::makePlan(const std::vector<DownloadItem>& in) const
{
    DownloadPlan p;
    p.filesystemFreeBytes = freeBytes();
    p.usableFreeBytes = p.filesystemFreeBytes > DOWNLOAD_SAFETY_RESERVE
    ...
    p.sizeKnown = true;
    for (auto i : in)
        if (seen.emplace(i.itemId, true).second) {
            auto old = std::find_if(s.items.begin(), s.items.end(),
                                    [&](const DownloadItem& x) { return x.itemId == i.itemId; });
            if (old != s.items.end()) {
                i.downloadedBytes = old->downloadedBytes;
                if (!i.hlsStorage && old->expectedSize)
                    i.expectedSize = old->expectedSize;
            }
            std::uint64_t planned = 0;
            if (!plannedDownloadBytes(i, planned)) {
                p.sizeKnown = false;
                p.items.push_back(i);
                continue;
            }
            ...
            p.alreadyPresentBytes =
                saturatingAdd(p.alreadyPresentBytes, std::min(i.downloadedBytes, planned));
            if (old == s.items.end())
                p.additionalRequiredBytes =
                    saturatingAdd(p.additionalRequiredBytes,
                                  i.downloadedBytes >= planned ? 0 : planned - i.downloadedBytes);
        }
    p.alreadyReservedBytes = s.reservedBytes;
    p.usableFreeBytes =
        p.usableFreeBytes > s.reservedBytes ? p.usableFreeBytes - s.reservedBytes : 0;
    p.canFit = p.sizeKnown && p.additionalRequiredBytes <= p.usableFreeBytes;
    if (!p.sizeKnown)
        p.error = "Estimated HLS size unavailable";
    else if (!p.canFit)
        p.error = "Not enough space";
    return p;
}
std::uint64_t DownloadManager::requestPlan(const std::vector<MediaItem>& items)
{
    ...
    for (const auto& m : items) {
        ...
        i.itemType = m.type;
        i.title = m.title;
        i.runtimeTicks = m.runTimeTicks;
        i.hlsStorage = true;
        provisional.push_back(i);
    }
    DownloadPlan estimate;
    {
        UiDiagnostics::Scope scope("DownloadManager::requestPlan snapshot/filesystem estimate",
                                   false);
...
std::uint64_t DownloadManager::requestSeriesPlan(const std::string& seriesId)
{
    MediaItem series;
    series.id = seriesId;
    return requestSeriesPlan(series);
}
std::uint64_t DownloadManager::requestSeasonPlan(const std::string& seriesId,
...
DownloadPlanSnapshot DownloadManager::planSnapshot(std::uint64_t id) const
{
    UiDiagnostics::Scope scope("DownloadManager::planSnapshot mutex wait");
    std::lock_guard<std::mutex> l(m_mutex);
    auto it = m_plans.find(id);
    return it == m_plans.end() ? DownloadPlanSnapshot{} : it->second;
}
bool DownloadManager::tryPlanSnapshot(std::uint64_t id, DownloadPlanSnapshot& snapshot) const
{
...
void DownloadManager::planner()
{
    for (;;) {
        PlanJob job;
        {
            ...
            m_planWake.wait(lock, [&] { return m_stop || !m_planJobs.empty(); });
            if (m_stop)
                return;
            ...
            publishDownloadGauges(m_items, m_planJobs.size());
            performanceTelemetry().setWorkerActive(WorkerId::DownloadPlanner, true);
        }
        ...
        auto queryEpisodes = [&](const std::string& seasonId, std::vector<MediaItem>& episodes) {
            if (!job.libraryQuery)
                return false;
        ...
        auto loadEpisodesThroughCoordinator = [&](const MediaItem& series, const MediaItem& season,
            ...
            for (;;) {
                ...
                if (!result.success) {
                    error = result.message.empty() ? "LibrarySync episode refresh failed"
                                                   : result.message;
                    return false;
                }
        ...
        auto loadSeasonsThroughCoordinator = [&](const MediaItem& series,
            ...
            for (;;) {
                ...
                if (!result.success) {
                    error = result.message.empty() ? "LibrarySync season refresh failed"
                                                   : result.message;
                    return false;
                }
        ...
        if (!job.seriesId.empty() && !job.seasonId.empty()) {
            ...
            if (!hierarchyReady && !hierarchySuperseded) {
                ...
                season.id = job.seasonId;
                season.seriesId = job.seriesId;
                hierarchyReady = loadEpisodesThroughCoordinator(job.series, season, media);
                if (hierarchyReady) {
                    seasons.push_back(std::move(season));
                    hierarchy[job.seasonId] = media;
                }
            ...
            if (job.libraryQuery) {
                ...
                if (cachedSeasons.superseded || cachedSeasons.cancelled) {
                    hierarchySuperseded = true;
                } else if (cachedSeasons.success && !cachedSeasons.items.empty()) {
                    bool allCached = true;
                    ...
                    for (const auto& season : seasons) {
                        ...
                        media.insert(media.end(), episodes.begin(), episodes.end());
                    }
            ...
            if (!hierarchyReady && !hierarchySuperseded && error.empty()) {
                ...
                media.clear();
                hierarchy.clear();
                hierarchyReady = loadSeasonsThroughCoordinator(job.series, seasons);
                if (hierarchyReady) {
                    for (const auto& season : seasons) {
                        std::vector<MediaItem> episodes;
                        if (!loadEpisodesThroughCoordinator(job.series, season, episodes)) {
        ...
        plan->second.itemCount = out.empty() && hierarchyReady ? media.size() : out.size();
        if (!error.empty()) {
