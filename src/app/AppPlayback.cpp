...
{
    static const char* const labels[] = {"Loading...", "Loading..", "Loading.",  "Loading",
...
void emitPlaybackEvent(PerformanceTelemetry& telemetry, uint32_t sequence, PlaybackStage stage,
                       PlaybackSourceKind source, uint8_t exitKind, int32_t exitCode,
                       uint64_t durationUs) noexcept
{
    if (!telemetry.enabledFast() || sequence == 0)
        return;
...
bool reapPlaybackChildWithin(pid_t pid, int& status, std::chrono::milliseconds timeout) noexcept
{
...
bool App::ingestPlaybackResult(bool removeAfterIngest)
{
    ...
    PlaybackResult r;
    std::string error;
    if (!PlaybackRequest::readResultFrom("playback-result.txt", r, error))
        return false;
    bool pending = false;
    if (!r.serverReported) {
        OfflinePlaybackEntry entry;
        entry.itemId = r.itemId;
        entry.itemType = r.itemType;
        entry.baseServerTicks = r.baseResumeTicks;
        entry.finalTicks = r.positionTicks;
        entry.localTimestamp = (std::uint64_t)time(nullptr);
        const std::string journal = OfflinePlaybackJournal::path(
    ...
    if (removeAfterIngest)
        PlaybackRequest::removeAt("playback-result.txt");
    return pending;
}
void App::recoverPlaybackResult()
{
    if (ingestPlaybackResult(true))
        scheduleJournalSync();
}
...
void App::scheduleJournalSync()
{
    ...
    if (!m_journalSyncThread.joinable())
        m_journalSyncThread = std::thread(&App::journalSyncLoop, this);
    {
        std::lock_guard<std::mutex> lock(m_journalMutex);
        m_journalWake = true;
    }
...
void App::journalSyncLoop()
{
    ...
    while (!m_journalStop) {
        ...
        if (failures) {
            ...
            m_journalCv.wait_for(lock, std::chrono::seconds(seconds),
                                 [this] { return m_journalStop || m_journalWake; });
            if (m_journalStop)
                break;
            m_journalWake = false;
        }
...
void App::syncPlaybackJournal()
{
    ...
    if (!session.valid())
        return;
    const std::string journal = OfflinePlaybackJournal::path(
        "cache", LibraryCache::scopeKey(session.serverUrl, session.userId));
    std::lock_guard<std::mutex> lock(m_journalMutex);
    std::vector<OfflinePlaybackEntry> entries;
    if (!OfflinePlaybackJournal::load(journal, entries, nullptr))
        return;
    auto map = [](PlaybackSyncStatus value) {
        switch (value) {
        case PlaybackSyncStatus::Success:
            return OfflineJournalRequestStatus::Success;
        case PlaybackSyncStatus::Unauthorized:
            return OfflineJournalRequestStatus::Unauthorized;
        case PlaybackSyncStatus::Missing:
            return OfflineJournalRequestStatus::Missing;
        default:
            return OfflineJournalRequestStatus::Transient;
        }
    };
    OfflineJournalSyncStats stats = syncOfflinePlaybackEntries(
        entries,
        [&](const OfflinePlaybackEntry& entry, std::int64_t& ticks) {
            std::string error;
            PlaybackSyncStatus status = PlaybackSyncStatus::Transient;
            RouteRequest(session).run(
                [&](const std::string& base) {
                    status = JellyfinApi::getPlaybackPositionTicks(base, session.accessToken,
                                                                   session.userId, session.deviceId,
                                                                   entry.itemId, ticks, error);
                    return status == PlaybackSyncStatus::Success;
                },
                error);
            return map(status);
        },
        [&](const OfflinePlaybackEntry& entry) {
            std::string error;
            PlaybackSyncStatus status = PlaybackSyncStatus::Transient;
            RouteRequest(session).run(
                [&](const std::string& base) {
                    status = JellyfinApi::reportPlaybackStopped(base, session.accessToken,
                                                                session.deviceId, entry.itemId,
                                                                entry.finalTicks, error);
                    return status == PlaybackSyncStatus::Success;
                },
                error);
            return map(status);
    ...
    if (stats.changed)
        OfflinePlaybackJournal::save(journal, entries, nullptr);
...
void App::handleExternalPlayback()
{
...
#if defined(MIYOOFIN_ENABLE_PERF_TELEMETRY) && MIYOOFIN_ENABLE_PERF_TELEMETRY == 1
    emitPlaybackEvent(telemetry, m_playbackSequence, PlaybackStage::SuspendPlatform,
                      telemetryPlaybackSource(m_playbackSource), 0, 0, suspendTimer.elapsedUs());
#endif
    ...
    telemetry.emitSessionEvent(SessionEventKind::SamplingSuspended, Outcome::Success,
...
#if defined(MIYOOFIN_ENABLE_PERF_TELEMETRY) && MIYOOFIN_ENABLE_PERF_TELEMETRY == 1
    const uint8_t childExitKind =
        waitedPid < 0 ? 0 : (WIFEXITED(status) ? 1 : (WIFSIGNALED(status) ? 2 : 0));
    const int32_t childExitCode = waitedPid >= 0 && WIFEXITED(status)
                                      ? WEXITSTATUS(status)
                                      : (WIFSIGNALED(status) ? WTERMSIG(status) : 0);
    emitPlaybackEvent(telemetry, m_playbackSequence, PlaybackStage::ChildWait,
    ...
    if (!resumePlatform()) {
        ...
        telemetry.emitSessionEvent(SessionEventKind::SamplingResumed, Outcome::Success,
                                   static_cast<uint32_t>(SamplingReason::ExternalPlayback), 0);
#endif
...
#if defined(MIYOOFIN_ENABLE_PERF_TELEMETRY) && MIYOOFIN_ENABLE_PERF_TELEMETRY == 1
    emitPlaybackEvent(telemetry, m_playbackSequence, PlaybackStage::ResumePlatform,
                      telemetryPlaybackSource(m_playbackSource), 0, 0, resumeTimer.elapsedUs());
    telemetry.suspendSampling(false, SamplingReason::ExternalPlayback);
    telemetry.emitSessionEvent(SessionEventKind::SamplingResumed, Outcome::Success,
                               static_cast<uint32_t>(SamplingReason::ExternalPlayback), 0);
...
void App::drawPlaybackStartingOverlay(Uint32 elapsedMs)
{
    ...
    BitmapFont::fillRect(m_fb, boxX, boxY, boxW, boxH, Theme::BG_R, Theme::BG_G, Theme::BG_B);
    BitmapFont::drawRect(m_fb, boxX, boxY, boxW, boxH, Theme::ACCENT_R, Theme::ACCENT_G,
                         Theme::ACCENT_B);
    BitmapFont::drawString(m_fb, boxX + paddingX, boxY + paddingY, label, Theme::TEXT_R,
