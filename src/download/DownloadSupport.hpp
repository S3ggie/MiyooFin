...
{
    char b[32];
    std::snprintf(b, sizeof(b), "S%02dE%02d ", i.seasonNumber, i.episodeNumber);
...
inline unsigned downloadPercent(const DownloadItem& i)
{
    ...
    return i.expectedSize
               ? (unsigned)std::min<std::uint64_t>(100, i.downloadedBytes * 100 / i.expectedSize)
               : 0;
}
...
inline std::uint64_t predictedDownloadTotalBytes(const DownloadItem& i)
{
    ...
    if (i.hlsSegmentCount && i.hlsCompletedSegments >= 1) {
        ...
        const std::uint64_t completedBytes =
            i.downloadedBytes >= current ? i.downloadedBytes - current : 0;
                                     ...
                                     std::uint64_t segmentBytes)
{
    ...
    item.hlsCompletedSegments =
        item.hlsSegmentCount ? std::min(completed, item.hlsSegmentCount) : completed;
}
...
inline bool transferSourceChanged(const DownloadItem& live, const std::string& transferSourceId,
...
inline TransferFinish decideTransferFinish(bool deleteRequested, bool sourceChanged, bool validated)
{
    if (deleteRequested)
        return TransferFinish::Removed;
    if (sourceChanged)
        return TransferFinish::StaleAborted;
    if (!validated)
        return TransferFinish::RetryFailed;
