...
{
    switch (value) {
    case CatalogDbMigrationDecision::CreateEmptyFinal:
        return "create_empty_final";
    case CatalogDbMigrationDecision::RebuildMigratingAtMigrationStart:
        return "rebuild_migrating";
    case CatalogDbMigrationDecision::FinalDatabaseWins:
        return "final_database_wins";
    case CatalogDbMigrationDecision::FinalDatabaseWinsCleanupCandidate:
        return "final_wins_cleanup";
    case CatalogDbMigrationDecision::PathError:
        return "path_error";
    }
    return "unknown";
}
...
{
    switch (value) {
    case CatalogDbScopeStatus::Unconfigured:
        return "unconfigured";
    case CatalogDbScopeStatus::Pending:
        return "pending";
    case CatalogDbScopeStatus::Ready:
        return "ready";
    case CatalogDbScopeStatus::InvalidIdentity:
        return "invalid_identity";
    case CatalogDbScopeStatus::OpenFailed:
        return "open_failed";
    }
    return "unknown";
}
...
{
    switch (value) {
    case CatalogDbOpenState::NotAttempted:
        return "not_attempted";
    case CatalogDbOpenState::CreatedV1:
        return "created_v1";
    case CatalogDbOpenState::SupportedV1:
        return "supported_v1";
    case CatalogDbOpenState::CreatedV2:
        return "created_v2";
    case CatalogDbOpenState::SupportedV2:
        return "supported_v2";
    case CatalogDbOpenState::CreatedV3:
        return "created_v3";
    case CatalogDbOpenState::SupportedV3:
        return "supported_v3";
    case CatalogDbOpenState::WrongApplicationId:
        return "wrong_application_id";
    case CatalogDbOpenState::UnsupportedVersion:
        return "unsupported_version";
    case CatalogDbOpenState::CorruptOrIo:
        return "corrupt_or_io";
    }
    return "unknown";
}
                                          ...
                                          CatalogDbOpenState openState)
{
    ...
    std::snprintf(
        ...
        ready ? 1 : 0, scopeStatusName(status), static_cast<unsigned>(status),
        errorCategoryName(error), static_cast<unsigned>(error), openStateName(openState),
...
inline bool pageTransactionShouldRetry(int sqliteRc, std::size_t attempt, std::size_t maxAttempts)
{
    if (attempt >= maxAttempts)
        return false;
    return sqliteRc == SQLITE_BUSY || sqliteRc == SQLITE_LOCKED || sqliteRc == SQLITE_FULL;
}
...
class ScopedSqliteStmt
{
  public:
    ...
    ScopedSqliteStmt(ScopedSqliteStmt&& other) noexcept : m_statement(other.m_statement)
    {
...
{
    return performanceTelemetry().enabledFast() ? TelemetryClock::monotonicUs() : 0;
}
                                      ...
                                      std::string& error)
{
    ...
    const int prepareRc =
        sqlite3_prepare_v2(db, "SELECT 1 FROM media_items WHERE id=?1", -1, &statement, nullptr);
    if (prepareRc != SQLITE_OK) {
        error = sqlite3_errmsg(db);
        return false;
    }
    const int bindRc = sqlite3_bind_text(statement, 1, id.c_str(), -1, SQLITE_TRANSIENT);
...
static inline bool collectObjects(sqlite3* db, std::set<std::string>& objects, std::string& error)
{
    sqlite3_stmt* statement = nullptr;
    const int rc =
...
static inline bool schemaConstraints(sqlite3* db, bool& foreignKeyCascade, bool& checkConstraints,
                                     std::string& error)
{
    ...
    const int foreignKeyRc = execResult(db,
                                        "INSERT INTO media_items(id, kind, season_id) "
                                        "VALUES('__task06_bad_fk__', 4, '__task06_missing__');",
                                        expectedError);
    const int foreignKeyCode = sqlite3_extended_errcode(db);
    foreignKeyCascade =
        (foreignKeyRc == SQLITE_CONSTRAINT || foreignKeyCode == SQLITE_CONSTRAINT_FOREIGNKEY) &&
        foreignKeyCode == SQLITE_CONSTRAINT_FOREIGNKEY;

    expectedError.clear();
    const int checkRc = execResult(db,
                                   "INSERT INTO media_items(id, kind) "
                                   "VALUES('__task06_bad_check__', 99);",
                                   expectedError);
    checkConstraints =
        checkRc == SQLITE_CONSTRAINT || sqlite3_extended_errcode(db) == SQLITE_CONSTRAINT_CHECK;

    if (!exec(db, "DELETE FROM media_items WHERE id='__task06_series__';", error)) {
        std::string ignored;
        exec(db, "ROLLBACK;", ignored);
        return false;
    }
...
static inline bool sameMediaItemScalars(const MediaItem& expected, const MediaItem& actual)
{
    return expected.id == actual.id && expected.type == actual.type &&
           expected.title == actual.title && expected.overview == actual.overview &&
           expected.year == actual.year && std::fabs(expected.rating - actual.rating) < 0.000001f &&
           ...
           expected.seriesName == actual.seriesName && expected.seriesId == actual.seriesId &&
           expected.seasonId == actual.seasonId;
}

static inline bool
validateHierarchyInput(const MediaItem& series, const std::vector<MediaItem>& seasons,
                       const std::map<std::string, std::vector<MediaItem>>& episodesBySeason,
                       std::string& error)
{
    if (series.id.empty() || !series.seriesId.empty() || !series.seasonId.empty()) {
    ...
    for (const auto& season : seasons) {
        if (season.id.empty() || season.type != "season" || season.seriesId != series.id ||
            !season.seasonId.empty() || !seasonIds.insert(season.id).second ||
    ...
    for (const auto& entry : episodesBySeason) {
        ...
        for (const auto& episode : entry.second) {
            if (episode.id.empty() || episode.type != "episode" || episode.seriesId != series.id ||
                episode.seasonId != entry.first || !itemIds.insert(episode.id).second) {
...
static inline bool validateReconcileInput(const std::vector<MediaItem>& series, std::string& error)
{
    ...
    for (const auto& item : series) {
        if (item.id.empty() || item.type != "show" || !item.seriesId.empty() ||
            !item.seasonId.empty() || !ids.insert(item.id).second) {
