...
void EpisodeBrowserScreen::startSelectedEpisodePlayback()
{
    ...
    std::string error;
    PlaybackSource source = m_downloads
                                ? resolvePlayback(m_episodes[m_selectedEpisode], *m_downloads)
    ...
    if (PlaybackRequest::writeWithSourceTo(
        ...
        m_stack->requestExternalPlayback(source == PlaybackSource::Local
...
{
    if (advancePrefetchResume(m_prefetchResumePending, m_prefetchResumeDelayUpdates)) {
        {
            std::lock_guard<std::mutex> lock(m_workerMutex);
        ...
        std::string error;
        if (PlaybackRequest::consumeResult(m_playbackEpisodeId, resultTicks, error)) {
            const int playedIndex = findEpisodeIndex(m_episodes, m_playbackEpisodeId);
            if (playedIndex >= 0) {
