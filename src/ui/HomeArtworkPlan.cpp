...
std::vector<HomePosterJob> planMediaPagePosterJobs(const std::vector<MediaItem>& items)
{
    ...
    std::sort(ordered.begin(), ordered.end(), [](const MediaItem* left, const MediaItem* right) {
...
std::vector<HomePosterJob> planHomePosterJobs(const LibrarySnapshot& snapshot)
{
    ...
    for (const auto& views : {&snapshot.movies, &snapshot.shows})
        for (const auto& view : *views)
            for (const auto& item : view.items)
                add(item);
    for (const auto& item : snapshot.continueWatching)
        add(item);
    for (const auto& item : snapshot.recentlyAdded)
        add(item);
    return out;
}
...
std::vector<HomePosterJob> planSeasonPosterJobs(const std::vector<MediaItem>& seasons)
{
    ...
    for (const auto& season : seasons) {
        if (!season.type.empty() && season.type != "season")
            continue;
