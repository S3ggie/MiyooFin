...
static size_t header_cb(char* buffer, size_t size, size_t nitems, void* /*ud*/)
{
    ...
    if (total > 5 && buffer[0] == 'H' && buffer[1] == 'T' && buffer[2] == 'T' && buffer[3] == 'P' &&
        buffer[4] == '/') {
    ...
    if (s_upstream.collecting) {
        ...
        if (!name.empty() && is_allowed_response_header(name.c_str(), name.size())) {
            ...
            if (name == "content-type")
                s_upstream.content_type = val;
            else if (name == "content-length")
                s_upstream.content_length = val;
            else if (name == "content-range")
                s_upstream.content_range = val;
            else if (name == "accept-ranges")
                s_upstream.accept_ranges = val;
        }
    }
    return total;
}
...
static std::string manifest_value(const std::string& body, const char* key)
{
    std::string p = std::string(key) + "=";
    size_t x = body.find(p);
    while (x != std::string::npos && x && body[x - 1] != '\n')
        x = body.find(p, x + 1);
    if (x == std::string::npos)
        return {};
    x += p.size();
    size_t e = body.find('\n', x);
    return body.substr(x, e == std::string::npos ? std::string::npos : e - x);
}
static bool local_number(const std::string& text, std::uint64_t& value)
{
    if (text.empty())
        return false;
    for (char c : text)
...
static bool local_manifest(const std::string& path, LocalMedia& m)
{
    ...
    char q[1024];
    size_t n;
    while (b.size() <= 65536 && (n = std::fread(q, 1, sizeof q, f)))
        b.append(q, n);
    std::fclose(f);
    if (b.size() > 65536)
        return false;
    ...
    if (b.rfind("MFDM=1\n", 0))
        return false;
    if (!local_number(manifest_value(b, "size"), m.size) ||
        !local_number(manifest_value(b, "chunk"), m.chunk) || !m.size || !m.chunk)
        return false;
    std::uint64_t count = 1 + (m.size - 1) / m.chunk;
    for (std::uint64_t i = 0; i < count; i++) {
        std::string name = m.itemDir + "/chunks/chunk-";
        char number[32];
        std::snprintf(number, sizeof number, "%06llu.bin", (unsigned long long)i);
        name += number;
        struct stat st
        {};
        std::uint64_t want = (i + 1 == count ? m.size - i * m.chunk : m.chunk);
        if (::stat(name.c_str(), &st) || st.st_size < 0 || (std::uint64_t)st.st_size != want)
            return false;
    }
    m.valid = true;
    return true;
}
static bool parse_local_range(const std::string& raw, std::uint64_t total, std::uint64_t& start,
                              std::uint64_t& end)
{
    if (raw.empty()) {
        start = 0;
    ...
    std::string s = parse_range_spec(raw);
    ...
    if (a.empty())
        return false;
    char* e = nullptr;
    start = std::strtoull(a.c_str(), &e, 10);
    if (!e || *e || start >= total)
        return false;
    if (b.empty())
        end = total - 1;
...
static bool local_segment_index(const std::string& path, const LocalMedia& m, std::uint64_t& index)
{
    const std::string prefix = "/segments/";
    if (path.compare(0, prefix.size(), prefix) != 0 || path.size() != prefix.size() + 6)
        return false;
    return local_number(path.substr(prefix.size()), index) && index < m.segments;
}
static void send_local_error(int fd, long code)
{
    std::string h = "HTTP/1.1 " + std::to_string(code) + " " + status_reason(code) +
                    "\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
    send_all(fd, h.c_str(), h.size());
}
static void send_local_body(int fd, const std::string& type, const std::string& data, bool head)
{
    std::string h = "HTTP/1.1 200 OK\r\nContent-Type: " + type +
                    "\r\nContent-Length: " + std::to_string(data.size()) +
                    "\r\nConnection: close\r\n\r\n";
    if (send_all(fd, h.c_str(), h.size()) && !head)
        send_all(fd, data.data(), data.size());
}
static void serve_local_segment(int fd, const LocalMedia& m, std::uint64_t index, bool head)
{
    char number[32];
    std::snprintf(number, sizeof number, "%06llu.bin", (unsigned long long)index);
    std::string name = m.itemDir + "/segments/" + number;
    struct stat st
    {};
    if (::lstat(name.c_str(), &st) || !S_ISREG(st.st_mode) || st.st_size <= 0) {
        send_local_error(fd, 404);
        return;
    }
    std::uint64_t length = (std::uint64_t)st.st_size;
    std::string h =
    ...
    while (left) {
        size_t want = (size_t)std::min<std::uint64_t>(left, sizeof buf),
               got = std::fread(buf, 1, want, f);
        if (got != want || !send_all(fd, buf, got))
            break;
        left -= got;
    }
    std::fclose(f);
}
static void handle_local_client(int fd, const LocalMedia& m)
{
    std::string d;
    char q[1024];
    while (d.size() < HTTPS_BRIDGE_MAX_HEADER_SIZE) {
    ...
    HttpRequest r = parse_request(d.data(), d.size());
    if (!r.valid || (r.method != "GET" && r.method != "HEAD")) {
        send_local_error(fd, 404);
        return;
    }
    if (m.kind == LocalMediaKind::Hls) {
        ...
        if (local_segment_index(r.path, m, index)) {
            serve_local_segment(fd, m, index, r.method == "HEAD");
            return;
        }
        send_local_error(fd, 404);
        return;
    ...
    if (!parse_local_range(r.range, m.size, a, b)) {
        std::string x = "HTTP/1.1 416 Range Not Satisfiable\r\nContent-Range: bytes */" +
                        std::to_string(m.size) +
                        "\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
        send_all(fd, x.c_str(), x.size());
        return;
    }
    std::uint64_t len = b - a + 1;
    std::string h = "HTTP/1.1 " + std::string(r.range.empty() ? "200 OK" : "206 Partial Content") +
                    "\r\nAccept-Ranges: bytes\r\nContent-Length: " + std::to_string(len) + "\r\n";
    if (!r.range.empty())
        h += "Content-Range: bytes " + std::to_string(a) + "-" + std::to_string(b) + "/" +
             std::to_string(m.size) + "\r\n";
    h += "Connection: close\r\n\r\n";
    if (!send_all(fd, h.c_str(), h.size()) || r.method == "HEAD")
        return;
    char buf[32768];
    std::uint64_t p = a, left = len;
    while (left) {
        std::uint64_t ci = p / m.chunk, off = p % m.chunk;
        char number[32];
        std::snprintf(number, sizeof number, "%06llu.bin", (unsigned long long)ci);
        std::string name = m.itemDir + "/chunks/chunk-" + number;
        FILE* f = std::fopen(name.c_str(), "rb");
        if (!f)
            return;
        std::fseek(f, (long)off, SEEK_SET);
        size_t want = (size_t)std::min<std::uint64_t>(
            left, std::min<std::uint64_t>(sizeof buf, m.chunk - off));
...
int main(int argc, char* argv[])
{
    ...
    if ((!local && !fallback && (argc < 3 || argc > 4)) || (fallback && (argc < 5 || argc > 6)) ||
        (local && (argc < 3 || argc > 4))) {
        usage(argv[0]);
        return 1;
    }
    ...
    if (!local && (uses_https(upstream_url) || uses_https(fallback_url)) &&
        ...
        std::fprintf(stderr,
                     "Error: a nonempty CA bundle is required for HTTPS upstream playback\n");
        return 1;
    }
    LocalMedia media;
    if (local && !local_manifest(argv[2], media)) {
        std::fprintf(stderr, "Error: invalid local manifest\n");
        return 1;
    }
    int port = 18080;
    ...
    while (g_running) {
        ...
        if (local)
            handle_local_client(cfd, media);
        else
            handle_client(cfd, upstream_url, fallback_url, cacert_path);
        ::close(cfd);
    }
