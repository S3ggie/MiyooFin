# Task 026 — Split JellyfinApi download/HLS endpoints

## Status

NOT STARTED

## Depends On

Task 025

## Goal

Split JellyfinApi download/HLS endpoints. This task must be behavior-preserving.

## Allowed Files

- `src/net/JellyfinApi.cpp`
- `src/net/JellyfinApiDownload.cpp`
- `Makefile`

## Forbidden Scope

Every file not listed under **Allowed Files** is forbidden for this task.
Do not edit other task files. Do not perform opportunistic cleanup.

## Pre-change Checks

Run these before editing:

```sh
git status --short
make test
```

`git status --short` must be empty. If either check fails, STOP without editing.

## Exact Steps

Create `JellyfinApiDownload.cpp`.
Move downloadable-media-source discovery, HLS playlist/segment URL handling, HLS failure classification, and download-specific JellyfinApi definitions.
Keep retry/failure policy users unchanged.

## Behavior That Must Not Change

- HLS URLs/profile remain identical.
- Failure classification remains identical.
- No TLS verification changes.
- No legacy direct-download behavior is reintroduced.

## Focused Validation

Run:

```sh
`make refactor-check`
```

## Required Final Validation

Run all of the following after the change:

```sh
make test
git diff --check
git status --short
git diff --stat
git diff
```

Read the full diff. Verify every changed path is listed under Allowed Files.

## Acceptance Criteria

- The stated extraction/change is complete.
- No unrelated behavior change is present.
- All required validation passes.
- Only Allowed Files changed.
- The worktree is clean after the commit.

## Commit

Commit exactly this task with:

```sh
git add src/net/JellyfinApi.cpp src/net/JellyfinApiDownload.cpp Makefile
git commit -m "refactor: split jellyfin download api"
git status --short
```

If an Allowed File did not change, do not force-add or manufacture a change.

## STOP

After the commit succeeds, STOP.
Do not read or execute the next task.
Report the changed files, validation commands, commit SHA, and any concern.
