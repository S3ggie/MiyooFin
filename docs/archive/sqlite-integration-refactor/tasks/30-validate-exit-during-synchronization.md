# Task 30 — Validate exit during synchronization

**Phase:** C — validation

## Objective

Prove active LibrarySync/CatalogDb work cancels/joins cleanly during teardown with no stale publication/detached worker.

## Why

Exit during sync is a required lifecycle gate after ownership moves.
...
- `tests/test_main.cpp` only if lifecycle fixture registration is needed
...
1. Start LibrarySync with cancellable in-flight work and staged generation.
2. Trigger component/App teardown through host test seam.
3. Observe cancellation and complete destruction.
4. Reopen catalog and inspect committed generation.

## Tests

- No deadlock/detached worker.
- Staged generation not finalized after teardown.
- Previous committed generation remains.
- CatalogDb closes normally.
...
No physical-device runtime is required here, but ARM compilation/verification is mandatory because the lifecycle wiring must cross-compile.
...
```text
test(library): cover exit during sync
```

Do not include any part of Task 31 in this commit.
